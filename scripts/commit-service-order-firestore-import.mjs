...
import commitTools from "../lib/service-order-firestore-commit.js";
...
const DEFAULT_PLAN_PATH = "tmp/service-order-firestore-write-plan.json";
const DEFAULT_OUTPUT_PATH = "tmp/service-order-firestore-commit-result.json";
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--allow-supersede-spreadsheet-events") {
      options.allowSupersedeSpreadsheetEvents = true;
      continue;
    }
...
function printHelp() {
  console.log("Usage: node scripts/commit-service-order-firestore-import.mjs --commit --confirm-source-import-id <id> [options]");
  console.log("");
  console.log("Options:");
  ...
  console.log("  --allow-planned-updates                  Allow updates to planned import-owned records.");
  console.log("  --allow-supersede-spreadsheet-events     Mark older spreadsheet events as superseded by PDF order events.");
  ...
  console.log("");
  console.log("This command creates/updates service-order import records. It never deletes, marks services completed, or matches songs.");
}
...
function buildResult({ plan, validation, classifications, writeResult, options }) {
  ...
  return {
    ...
    safety: {
      ...
      spreadsheetEventsSuperseded: summary.spreadsheetSongEventsSuperseded
    }
  };
}

function printResult(result, outputPath) {
  console.log("Service order Firestore commit result");
  ...
  console.log(`Service order items created: ${result.summary.serviceOrderItemsCreated}`);
  ...
  console.log(`Spreadsheet song events superseded: ${result.summary.spreadsheetSongEventsSuperseded}`);
  console.log(`Service moments created: ${result.summary.serviceMomentsCreated}`);
...
async function main() {
  ...
  const validation = validateServiceOrderCommitPlan(plan, {
    commit: options.commit,
    confirmSourceImportId: options.confirmSourceImportId,
    allowPlannedUpdates: options.allowPlannedUpdates,
    allowSupersedeSpreadsheetEvents: options.allowSupersedeSpreadsheetEvents
  });
