...
try{
 ...
 const bytes=await sharp({create:{width:32,height:32,channels:3,background:'#12587b'}}).png().toBuffer();
 await bucket.file(sourcePath).save(bytes,{resumable:false,metadata:{contentType:'image/png'}});
 const signed=JSON.parse(execFileSync('gcloud',['storage','sign-url',`gs://${bucketName}/${sourcePath}`,'--duration=10m','--impersonate-service-account='+service.spec.template.spec.serviceAccountName,'--format=json'],{encoding:'utf8',stdio:['ignore','pipe','pipe']}));
 const sourceUrl=signed[0]?.signed_url||signed.signed_url;check(sourceUrl,'No source URL');
 const upload={kind:'image',title:prefix+' image',folderId:ids[0],description:'Temporary system verification fixture, not historical evidence.',sourceType:'generated',prompt:'Programmatically generated blue test square.',file:{file_id:prefix,download_link:sourceUrl},idempotencyKey:prefix+'-image'};
