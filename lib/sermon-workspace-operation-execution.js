...
} = require("./sermon-workspace-operation-registry");
...
function getExecutionsCollection(deps = {}) {
  const collection = deps.sermonOperationExecutionsCollection;
  if (!collection || typeof collection.doc !== "function") {
    throw createExecutionError(
      "Sermon operation executions collection is not configured",
      500,
      "sermon_operation_executions_not_configured"
    );
  }
  return collection;
}
...
function buildReplaySummary(result = {}) {
  ...
  while (queue.length > 0 && Object.keys(identifiers).length < 40) {
    ...
    for (const [key, child] of Object.entries(value)) {
      if (
        typeof child === "string" &&
        (/Id$/.test(key) || ["status", "filename", "downloadUrl", "downloadUrlExpiresAt"].includes(key))
      ) {
        identifiers[key] = child;
      } else if (
        typeof child === "number" &&
        ["count", "slideCount", "sizeBytes"].includes(key)
      ) {
        identifiers[key] = child;
...
async function runIdempotentSermonWorkspaceOperation(input = {}, deps = {}) {
  ...
  const executionId = `sermon-operation-${hashValue(`${mode}\u0000${operation}\u0000${idempotencyKey}`).slice(0, 40)}`;
