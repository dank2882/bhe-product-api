...
async function runIdempotentPrayerManagementOperation(input = {}, deps = {}) {
  ...
  const pending = { executionId, ownerSub: identity, actorSub: authorizedOwner.actorSubject, operation: String(input.operation || ""), requestFingerprint: fingerprint, idempotencyKeyHash: keyHash, argumentKeys: Object.keys(input.arguments || {}).sort(), status: "in_progress", createdAt: startedAt, updatedAt: startedAt, contentRedacted: true };
