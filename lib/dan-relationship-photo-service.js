...
async function deletePrivateBuffer(bucket, storageKey) {
...
async function adjustRelationshipPhotoCrop(input = {}, deps = {}) {
  ...
  const candidatePreviewStorageKey = `relationships/${existing.personId}/photos/${existing.photoId}/preview-candidates/${randomUUID()}.jpg`;
  await savePrivateBuffer(bucket, candidatePreviewStorageKey, rendered.buffer, "image/jpeg");
  const docRef = photos.doc(existing.photoId);
  let next;
  try {
    next = await runDanFirestoreTransaction(deps, photos, async (transaction) => {
      ...
      const updated = {
        ...current,
        previewStorageKey: candidatePreviewStorageKey,
        crop: rendered.crop,
        sourceMetadata: rendered.sourceMetadata,
        warnings: rendered.warnings,
        status: "preview_ready",
    ...
    await deletePrivateBuffer(bucket, candidatePreviewStorageKey).catch(() => {});
    throw error;
  }
...
async function approveRelationshipProfilePhoto(input = {}, deps = {}) {
  ...
  const prefix = `relationships/${person.personId}/photos/${existing.photoId}/approved-candidates/${candidateId}`;
  ...
  let promoted;
  try {
    promoted = await runDanFirestoreTransaction(deps, photos, async (transaction) => {
    ...
    await Promise.all([
      deletePrivateBuffer(bucket, thumbnailStorageKey).catch(() => {}),
      deletePrivateBuffer(bucket, outlookStorageKey).catch(() => {})
    ]);
    throw error;
  }
  const { nextPhoto, nextPerson } = promoted;
