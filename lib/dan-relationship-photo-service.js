...
async function adjustRelationshipPhotoCrop(input = {}, deps = {}) {
  ...
  await photos.doc(existing.photoId).set(next);
...
async function approveRelationshipProfilePhoto(input = {}, deps = {}) {
  ...
  const nextPerson = {
    ...person,
    profilePhotoId: existing.photoId,
    version: Number(person.version || 0) + 1,
    updatedAt: now
  };
  await photos.doc(existing.photoId).set(nextPhoto);
