...
function normalizeServiceMessage(message = {}) {
  ...
  if (message.sourceCells && typeof message.sourceCells === "object" && !Array.isArray(message.sourceCells)) {
    const sourceCells = {};

    for (const [fieldName, sourceCell] of Object.entries(message.sourceCells)) {
      const cleanSourceCell = normalizeString(sourceCell);
      if (cleanSourceCell) {
        sourceCells[fieldName] = cleanSourceCell;
      }
    }

    if (Object.keys(sourceCells).length > 0) {
      result.sourceCells = sourceCells;
    }
  }
