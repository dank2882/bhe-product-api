...
function parseScheduleText(value) {
  ...
  if (!text || text === "unscheduled" || text === "daily" || text === "every day") return { kind: "daily", timeZone };
  const monthly = text.match(/(?:every\s+month|monthly)(?:\s+on)?\s+(?:day\s+)?(\d{1,2})\b/);
  if (monthly) {
    const dayOfMonth = Number(monthly[1]);
    if (dayOfMonth >= 1 && dayOfMonth <= 31) return { kind: "monthly", dayOfMonth, timeZone };
    return null;
  }
  ...
  const weekInterval = text.match(/every\s+(\d+)\s+weeks?/);
  if (weekInterval && date) return { kind: "interval", intervalDays: Number(weekInterval[1]) * 7, startDate: date[1], timeZone };
  if (weekInterval) return null;
  if (date) return { kind: "date", date: date[1], timeZone };
  const weekdayPatterns = [
    /\bsun(?:day)?s?\b/, /\bmon(?:day)?s?\b/, /\btue(?:s(?:day)?)?s?\b/,
    /\bwed(?:nesday)?s?\b/, /\bthu(?:rsday)?s?\b/, /\bfri(?:day)?s?\b/,
    /\bsat(?:urday)?s?\b/
  ];
  const weekdays = weekdayPatterns.map((pattern, index) => pattern.test(text) ? index : -1).filter((index) => index >= 0);
