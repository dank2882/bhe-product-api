...
async function attachTaskFile(input = {}, deps = {}) {
  ...
  try {
    ...
    // Concurrent identical uploads share an immutable object. Never delete the
    // winner's file on a Firestore create conflict or an ambiguous write result.
    const winner = await docRef.get();
    if (winner.exists) return { action: "existing", attachment: buildAttachmentSummary(winner.data() || {}, attachmentId) };
    throw error;
  }
...
async function getTaskAttachmentDownload(input = {}, deps = {}) {
  ...
  // Keep the existing download contract. Raster images also get an inline link
  // for the embedded photo viewer without forcing a file download.
  let preview;
  if (["image/jpeg", "image/png", "image/gif", "image/webp"].includes(attachment.contentType)) {
    const [previewUrl] = await deps.taskAttachmentBucket.file(attachment.storagePath).getSignedUrl({
      version: "v4", action: "read", expires: expiresAtMs,
      responseDisposition: "inline", responseType: attachment.contentType
    });
    preview = { url: previewUrl, expiresAt: new Date(expiresAtMs).toISOString() };
  }
  return {
    attachment: buildAttachmentSummary(attachment, attachmentId),
    download: { url, expiresAt: new Date(expiresAtMs).toISOString() },
    ...(preview ? { preview, guidance: "For Maintenance photo previews use tasks_open_maintenance_photos with the task reference. Markdown image embedding failed ChatGPT acceptance. The preview URL supports the embedded viewer or direct opening; retain the download link. Links expire after 15 minutes; retrieve fresh links when needed." } : {})
  };
}
...
for (const name of ["attachTaskFile", "listTaskAttachments", "getTaskAttachmentDownload"]) {
  const handler = module.exports[name];
  module.exports[name] = async (input, deps = {}) => handler(input, await withProjectContext(deps));
}
