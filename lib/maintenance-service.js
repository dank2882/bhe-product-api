...
async function listMaintenanceBoard(input = {}, deps) {
  ...
  const { projectRow, presentBoard, renderTable } = require("./maintenance-board-presentation");
  // Reuse the complete, permission-filtered task read before grouping/paging.
  const tasks = await listTasks({ projectId: ROOT_ID, includeDescendants: true, includeArchived: input.includeArchived === true }, deps, true);
  const attachments = deps.taskAttachmentsCollection ? await readCompleteQuery(deps.taskAttachmentsCollection) : [];
  const images = new Map();
  for (const doc of attachments) {
    const value = doc.data();
    if (value.recordType !== "task" || !value.contentType?.startsWith("image/")) continue;
    if (!images.has(value.recordId)) images.set(value.recordId, []);
    images.get(value.recordId).push({ attachmentId: doc.id, fileName: value.fileName });
  }
  for (const photos of images.values()) photos.sort((a, b) => a.attachmentId.localeCompare(b.attachmentId));
  const board = presentBoard(tasks.tasks.map(task => projectRow(task, deps, images.get(task.taskId) || [], input.detailLevel === "full")), input);
  // Sign only visible authorized rows, after the stable pagination snapshot is
  // computed. Expiring links must never invalidate an otherwise valid cursor.
  const { getTaskAttachmentDownload } = require("./task-attachment-service");
  const photoRows = board.rows.filter(row => row.columns.Images.length);
  for (const row of photoRows) row.columns.Images[0].previewStatus = "on_request";
  const selected = photoRows.slice(0, 20);
  for (let start = 0; start < selected.length; start += 4) {
    await Promise.all(selected.slice(start, start + 4).map(async row => {
      const photo = row.columns.Images[0];
      try {
        const result = await getTaskAttachmentDownload({ attachmentId: photo.attachmentId }, deps);
        Object.assign(photo, { download: result.download, ...(result.preview ? { preview: result.preview } : {}), previewStatus: result.preview ? "ready" : "unavailable" });
      } catch (error) {
        // A revoked permission must fail closed. A signing/storage failure must
        // not hide the work list or incorrectly report that no photos exist.
        if ([401, 403].includes(error.statusCode)) throw error;
        photo.previewStatus = "unavailable";
      }
    }));
  }
  board.markdown = renderTable(board.rows, board.range, board.totalCount, board.hasMore, false);
  return board;
}

async function listMaintenanceRoutines(input = {}, deps) {
  const c = await context(deps); deps = c.deps;
  const { projectRow, presentBoard } = require("./maintenance-board-presentation");
  ...
  const board = presentBoard(routines.map(routine => projectRow(routine, deps, [], input.detailLevel === "full", true)), { ...input, limit: input.limit ?? 100 }, true);
  const byId = new Map(routines.map(routine => [routine.routineId, routine]));
  return { ...board, routines: board.rows.map(row => {
    const routine = byId.get(row.routineId);
    return input.detailLevel === "full" ? routine : { routineId: row.routineId, version: row.version, projectId: routine.projectId, title: routine.title, status: routine.status, assignedTo: routine.assignedTo, recurrence: routine.recurrence, recurrenceNotes: routine.recurrenceNotes, lastCompletedAt: routine.lastCompletedAt || "", notes: row.columns.Notes };
  }) };
}
