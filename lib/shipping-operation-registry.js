...
const definitions = [
  ["listCountryCostEstimates", "query", "Browse reusable country shipping budgets with separate currencies, historical pricing, unknown costs and refundable deposits. Follow all cursors.", [], ["country", "status", "limit", "cursor"]],
  ["getCountryCostEstimate", "query", "Read a country/route/container-size estimate sheet, source snapshots, missing costs and separate-currency totals; optional immutable version.", ["estimateId"], ["version"]],
  ["saveCountryCostEstimate", "command", "Create (expectedVersion 0) or replace a draft country budget. Full estimate schema below. Historical amounts must match source invoice lines; never a booking quote or payment.", ["estimateId", "expectedVersion", "estimate"], []],
...
const fieldGuide = {
  countryEstimate: "saveCountryCostEstimate requires estimateId, expectedVersion (0 for new) and estimate:{title,destinationCountry,route,containerSize(20|40|40HC|other|unknown),containerCount(1–100),asOf YYYY-MM-DD,status(draft|archived),coverage(partial|complete),assumptions,exclusions,lines[],historicalReferences:[{shipmentId,costId}]}; optional destinationCity. Replace the complete editable estimate; omit server fields and expanded snapshots on update. Multiple routes/sizes need separate stable estimate IDs. No current-market rates are inferred from history.",
  countryEstimateLines: "id,description,category(origin|ocean|documents|insurance|customs|port|brokerage|delivery|delay|contingency|deposit|other),currency,lowMinor,expectedMinor,highMinor,required(boolean),basis(historical|quote|allowance|unknown),note. Amounts are TOTALS for the whole stated container count; use null for unknown, never zero. Historical/quote also require reference:{shipmentId,costId},sourceLineId; expected amount must exactly match source currency, country, container size/count and non-included line. Invoices support historical basis; current quotes need expiry. Reimbursements are references only. Allowance needs a clear reason in note. Optional sourceDate. Low <= expected <= high; don't invent ranges. Deposit means refundable. Source dates/snapshots are added by server. Budgets never satisfy shipment readiness; keep currencies separate and follow up missing scope.",
...
function listShippingOperations(input = {}, deps) {
  ...
  return { catalogVersion: "1.2.0", owner: "fbc", domain: "shipping-containers", access: "Dan-only initial rollout; no inherited staff administrator access", operations: operations.filter(o => (!input.mode || o.mode === input.mode) && (!input.query || (o.name + " " + o.summary).toLowerCase().includes(input.query.toLowerCase()))), fieldGuide };
}
async function runShippingOperation(input, deps) {
  ...
  const result = input.operation === "createDocumentUpload" ? await service.createDocumentUpload(args, input.idempotencyKey, deps)
    : input.operation === "saveCountryCostEstimate" ? await service.saveCountryCostEstimate(args, input.idempotencyKey, deps)
