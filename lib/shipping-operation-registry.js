...
function listShippingOperations(input = {}, deps) {
  ...
  return { catalogVersion: "1.0.0", owner: "fbc", domain: "shipping-containers", access: "Dan-only initial rollout; no inherited staff administrator access", operations: operations.filter(o => (!input.mode || o.mode === input.mode) && (!input.query || (o.name + " " + o.summary).toLowerCase().includes(input.query.toLowerCase()))), fieldGuide };
}
async function runShippingOperation(input, deps) {
  ...
  const result = input.operation === "commitImportBatch" ? await service.commitImportBatch(args, input.idempotencyKey, deps)
