...
function normalizeSermonSeriesMessageMap(value, seriesId = "") {
  ...
  return value.map((item, index) => {
    ...
    const seriesNumber = Number.parseInt(String(item.seriesNumber ?? item.number ?? ""), 10);
...
async function getRequiredSermonSeriesDoc(input = {}, deps = {}) {
  ...
  const match = findMatchingSermonSeriesRecord(await loadCollection(foldersCollection, 10000), input);
  if (!match) {
    throw createSermonWorkspaceError(
...
async function resolveSeriesAwareFolderId(input = {}, series = {}, deps = {}) {
  ...
  const match = findMatchingSermonSeriesRecord(
    await loadCollection(getFoldersCollection(deps), 10000),
    series
  );
...
async function createSermonSeries(input = {}, deps = {}) {
  ...
  const records = await loadCollection(foldersCollection, 10000);
  ...
  if (matching) {
    ...
    adopted.version = 1;
    await foldersCollection.doc(matching.id).set(adopted);
...
async function listSermonSeries(input = {}, deps = {}) {
  ...
  const query = normalizeString(input.query).toLowerCase();
  const limit = normalizeLimit(input.limit);
  const series = (await loadCollection(getFoldersCollection(deps), 10000))
