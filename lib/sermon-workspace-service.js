...
const SERMON_SERIES_STATUSES = ["exploring", "planned", "active", "paused", "completed", "archived"];
const SERMON_SERIES_PREACHING_APPROACHES = [
  "expository",
  "text_driven_topical",
  "topical",
  "doctrinal",
...
const SERMON_SERIES_MESSAGE_STATUSES = [
  "provisional",
  "planned",
  "developing",
  "ready",
...
const MAX_SERMON_SERIES_MESSAGE_MAP_ITEMS = 100;
const MAX_SERMON_SERIES_DEVELOPMENT_TURNS = 100;
const MAX_SERMON_SERIES_DEVELOPMENT_TEXT_LENGTH = 500000;
...
function mapSermonSeriesStatusToFolderStatus(seriesStatus = "") {
  if (seriesStatus === "paused") return "paused";
  if (["completed", "archived"].includes(seriesStatus)) return "archived";
  return "active";
}

function inferSermonSeriesStatus(folder = {}) {
  const explicit = normalizeString(folder.seriesStatus);
  if (SERMON_SERIES_STATUSES.includes(explicit)) return explicit;
  if (folder.status === "paused") return "paused";
  if (folder.status === "archived") return "archived";
  return "active";
}

function normalizeSermonSeriesMessageMap(value, seriesId = "") {
  if (value === undefined || value === null) return [];
  if (!Array.isArray(value) || value.length > MAX_SERMON_SERIES_MESSAGE_MAP_ITEMS) {
    throw createSermonWorkspaceError(
      `Series message map must contain at most ${MAX_SERMON_SERIES_MESSAGE_MAP_ITEMS} items`,
      400,
      { maximumItems: MAX_SERMON_SERIES_MESSAGE_MAP_ITEMS },
      "invalid_sermon_series_message_map"
    );
  }
  ...
  return value.map((item, index) => {
    if (!isPlainObject(item)) {
      throw createSermonWorkspaceError(
        "Every series message-map item must be an object",
        400,
        { index },
        "invalid_sermon_series_message_map_item"
      );
    }
    const seriesNumber = Number(item.seriesNumber ?? item.number ?? "");
    if (!Number.isInteger(seriesNumber) || seriesNumber < 1 || usedNumbers.has(seriesNumber)) {
      throw createSermonWorkspaceError(
        "Every series message-map item requires a unique positive seriesNumber",
        400,
        { index, seriesNumber: item.seriesNumber ?? item.number ?? null },
        "invalid_sermon_series_message_number"
      );
    }
    usedNumbers.add(seriesNumber);
    const title = normalizeString(item.title);
    const scriptureText = normalizeString(item.scriptureText);
    const focus = normalizeString(item.focus);
    const sermonId = normalizeString(item.sermonId)
      ? validateDocId(item.sermonId, "sermonId")
      : "";
    if (!title && !scriptureText && !focus && !sermonId) {
      throw createSermonWorkspaceError(
        "A series message-map item needs a title, passage, focus, or linked sermonId",
        400,
        { index, seriesNumber },
        "empty_sermon_series_message_map_item"
      );
    }
    const mapItemId = normalizeString(item.mapItemId)
      ? validateDocId(item.mapItemId, "mapItemId")
      : `series-message-${slugify(seriesId || "series")}-${seriesNumber}`;
    if (usedIds.has(mapItemId)) {
      throw createSermonWorkspaceError(
        "Series message-map item IDs must be unique",
        400,
        { mapItemId },
        "duplicate_sermon_series_message_map_item_id"
      );
    }
    usedIds.add(mapItemId);
    return {
      mapItemId,
      seriesNumber,
      title,
      scriptureText,
      focus,
      intendedResponse: normalizeString(item.intendedResponse),
      targetDate: normalizeOptionalDate(item.targetDate, "seriesMessageTargetDate"),
      status: normalizeEnum(
        item.status,
        SERMON_SERIES_MESSAGE_STATUSES,
        sermonId ? "planned" : "provisional",
        "sermon_series_message_status"
      ),
      sermonId,
      notes: normalizeString(item.notes)
    };
  }).sort((left, right) => left.seriesNumber - right.seriesNumber);
}

function buildSermonSeriesSearchText(series = {}) {
  return [
    series.seriesId,
    series.folderId,
    series.seriesTitle,
    ...
    series.seriesStatus,
    series.description,
    series.pastoralBurden,
    series.intendedResponse,
    series.preachingApproach,
    series.preachingApproachNotes,
    series.scope,
    ...(Array.isArray(series.boundaries) ? series.boundaries : []),
    series.anchorScripture,
    series.audience,
    series.service,
    ...(Array.isArray(series.tags) ? series.tags : []),
    series.sourceIdea,
    series.notes,
    ...(Array.isArray(series.messageMap)
      ? series.messageMap.map((item) => [
          item.seriesNumber,
          item.title,
          item.scriptureText,
          item.focus,
          item.intendedResponse,
          item.notes
        ].filter(Boolean).join(" "))
      : []),
    ...(Array.isArray(series.developmentTurns)
      ? series.developmentTurns.map((turn) => turn.transcript)
      : [])
  ].filter(Boolean).join(" ").toLowerCase();
}

function buildSermonSeriesSummary(folder = {}, fallbackId = "") {
  const seriesTitle = normalizeString(folder.seriesTitle || folder.name);
  const seriesSlug = normalizeString(folder.seriesSlug) || normalizeSeriesSlug(seriesTitle);
  const seriesId = normalizeString(folder.seriesId) || (seriesSlug ? `series-${seriesSlug}` : "");
  return {
    folderId: normalizeString(folder.folderId || fallbackId),
    seriesId,
    seriesTitle,
    seriesSlug,
    version: Number.isInteger(Number(folder.version)) ? Number(folder.version) : 0,
    status: inferSermonSeriesStatus(folder),
    description: normalizeString(folder.description),
    pastoralBurden: normalizeString(folder.pastoralBurden),
    intendedResponse: normalizeString(folder.intendedResponse),
    preachingApproach: normalizeString(folder.preachingApproach),
    anchorScripture: normalizeString(folder.anchorScripture || folder.scriptureScope),
    audience: normalizeString(folder.audience),
    service: normalizeString(folder.service),
    targetStartDate: normalizeString(folder.targetStartDate),
    targetEndDate: normalizeString(folder.targetEndDate),
    messageCount: Array.isArray(folder.messageMap) ? folder.messageMap.length : 0,
    developmentTurnCount: Array.isArray(folder.developmentTurns) ? folder.developmentTurns.length : 0,
    createdAt: normalizeString(folder.createdAt),
    updatedAt: normalizeString(folder.updatedAt)
  };
}

function buildSermonSeriesDetail(folder = {}, fallbackId = "") {
  const summary = buildSermonSeriesSummary(folder, fallbackId);
  return {
    ...summary,
    preachingApproachNotes: normalizeString(folder.preachingApproachNotes),
    scope: normalizeString(folder.scope),
    boundaries: normalizeStringArray(folder.boundaries),
    messageMap: normalizeSermonSeriesMessageMap(folder.messageMap || [], summary.seriesId),
    tags: normalizeTags(folder.tags),
    sourceIdea: typeof folder.sourceIdea === "string" ? folder.sourceIdea : "",
    sourceIdeaSha256: normalizeString(folder.sourceIdeaSha256),
    sourceRefs: normalizeSourceRefs(folder.sourceRefs),
    notes: normalizeString(folder.notes),
    developmentTurns: Array.isArray(folder.developmentTurns)
      ? folder.developmentTurns.filter(isPlainObject).map(clone)
      : []
  };
}
...
function mergeSourceRefs(existingValue, incomingValue) {
  ...
  const merged = normalizeSourceRefs(existingValue);

  for (const incoming of normalizeSourceRefs(incomingValue)) {
    const type = normalizeString(incoming.type);
    if (type && replaceableTypes.has(type)) {
      const index = merged.findIndex((current) => normalizeString(current.type) === type);
      if (index >= 0) {
    ...
    if (!merged.some((current) => JSON.stringify(current) === fingerprint)) {
      merged.push(incoming);
    }
  }
...
function mergeTagsCaseInsensitive(...values) {
  const tags = [];
  const seen = new Set();
  for (const tag of values.flatMap(normalizeTags)) {
...
function assertExpectedImportRefreshState({
  sermonId,
  sourceId,
  currentSermon,
  currentSource,
  ...
  const staleFields = [];
  if (
    normalizeString(expectedSermonUpdatedAt) &&
    normalizeString(currentSermon?.updatedAt) !== normalizeString(expectedSermonUpdatedAt)
  ) {
    staleFields.push("sermon");
  }
  if (
    normalizeString(expectedSourceUpdatedAt) &&
    normalizeString(currentSource?.updatedAt) !== normalizeString(expectedSourceUpdatedAt)
  ) {
    staleFields.push("source");
  }
  if (staleFields.length > 0) {
    throw createSermonWorkspaceError(
      "The sermon import refresh plan is stale; run the read-only audit again",
      409,
      {
        sermonId,
        sourceId,
        staleFields,
        expectedSermonUpdatedAt: normalizeString(expectedSermonUpdatedAt),
        currentSermonUpdatedAt: normalizeString(currentSermon?.updatedAt),
        expectedSourceUpdatedAt: normalizeString(expectedSourceUpdatedAt),
        currentSourceUpdatedAt: normalizeString(currentSource?.updatedAt)
      },
      "stale_sermon_import_refresh_plan"
    );
  }
}

async function commitSermonImportRefresh({
  sermonId,
  sourceId,
  currentSermon,
  currentSource,
  ...
  if (!sermonChanged && !sourceChanged) return;

  const transactionRunner = deps.runSermonImportRefreshTransaction;
  if (typeof transactionRunner === "function") {
    const result = await transactionRunner({
      sermonId,
      sourceId,
      expectedSermonUpdatedAt: normalizeString(currentSermon.updatedAt),
      expectedSourceUpdatedAt: normalizeString(currentSource.updatedAt),
      nextSermon: sermonChanged ? nextSermon : null,
      nextSource: sourceChanged ? nextSource : null,
      snapshotWrite
    });
    if (result?.committed === true) return;
    throw createSermonWorkspaceError(
      "The sermon or Logos source changed during reconciliation; run the read-only audit again",
      409,
      {
        sermonId,
        sourceId,
        expectedSermonUpdatedAt: normalizeString(currentSermon.updatedAt),
        currentSermonUpdatedAt: normalizeString(result?.currentSermonUpdatedAt),
        expectedSourceUpdatedAt: normalizeString(currentSource.updatedAt),
        currentSourceUpdatedAt: normalizeString(result?.currentSourceUpdatedAt)
      },
      "concurrent_sermon_import_refresh"
    );
  }

  const sermonsCollection = getSermonsCollection(deps);
  const sourcesCollection = getSermonSourcesCollection(deps);
  const [freshSermonDoc, freshSourceDoc] = await Promise.all([
    sermonsCollection.doc(sermonId).get(),
    sourcesCollection.doc(sourceId).get()
  ]);
  const freshSermon = freshSermonDoc.exists ? (freshSermonDoc.data() || {}) : {};
  const freshSource = freshSourceDoc.exists ? (freshSourceDoc.data() || {}) : {};
  if (
    !freshSermonDoc.exists ||
    !freshSourceDoc.exists ||
    normalizeString(freshSermon.updatedAt) !== normalizeString(currentSermon.updatedAt) ||
    normalizeString(freshSource.updatedAt) !== normalizeString(currentSource.updatedAt)
  ) {
    throw createSermonWorkspaceError(
      "The sermon or Logos source changed during reconciliation; run the read-only audit again",
      409,
      {
        ...
        sourceId,
        expectedSermonUpdatedAt: normalizeString(currentSermon.updatedAt),
        currentSermonUpdatedAt: normalizeString(freshSermon.updatedAt),
        expectedSourceUpdatedAt: normalizeString(currentSource.updatedAt),
        currentSourceUpdatedAt: normalizeString(freshSource.updatedAt)
      },
      "concurrent_sermon_import_refresh"
    );
  }
  ...
  if (snapshotWrite) {
    await getSermonSnapshotsCollection(deps)
      .doc(snapshotWrite.snapshotId)
      .set(snapshotWrite.snapshot);
  }
  if (sermonChanged) await sermonsCollection.doc(sermonId).set(nextSermon);
  if (sourceChanged) await sourcesCollection.doc(sourceId).set(nextSource);
}
...
function normalizePreachingContextGuidance(value) {
  return (Array.isArray(value) ? value : [])
    .filter(isPlainObject)
    .map((item) => ({
      context: normalizeString(item.context || item.occasion),
      guidance: normalizeString(item.guidance || item.description),
...
function normalizePreachingGrowthGoals(value) {
  return (Array.isArray(value) ? value : [])
    .filter(isPlainObject)
    .map((item) => ({
      dimension: normalizeString(item.dimension || item.category),
      currentPattern: normalizeString(item.currentPattern || item.observation),
      nextGrowthTarget: normalizeString(item.nextGrowthTarget || item.target || item.practice),
      confidence: normalizeEnum(
        item.confidence,
        PROFILE_CONFIDENCE_LEVELS,
        "observed_once",
...
function normalizePreachingEvidenceCorpus(value) {
  const corpus = isPlainObject(value) ? value : {};
  return {
    asOfDate: normalizeString(corpus.asOfDate),
    currentSermonIds: normalizeStringArray(corpus.currentSermonIds).slice(0, 50),
    historicalSermonIds: normalizeStringArray(corpus.historicalSermonIds).slice(0, 50),
    analysisIds: normalizeStringArray(corpus.analysisIds).slice(0, 100),
    transcriptSourceIds: normalizeStringArray(corpus.transcriptSourceIds).slice(0, 100),
    notes: normalizeString(corpus.notes)
  };
}

function buildPreachingProfileFingerprint(profile = {}) {
  const stableProfile = {
    profileId: normalizeString(profile.profileId) || DEFAULT_PREACHING_PROFILE_ID,
    version: Math.max(Number(profile.version) || 0, 0),
    summary: normalizeString(profile.summary),
    tone: normalizeStringArray(profile.tone),
    strengths: normalizeStringArray(profile.strengths),
    recurringPatterns: normalizeStringArray(profile.recurringPatterns),
    cautions: normalizeStringArray(profile.cautions),
    draftingGuidance: normalizeString(profile.draftingGuidance),
    avoidances: normalizeStringArray(profile.avoidances),
    contextGuidance: normalizePreachingContextGuidance(profile.contextGuidance),
    growthGoals: normalizePreachingGrowthGoals(profile.growthGoals),
    observations: normalizeProfileObservations(profile.observations),
    evidenceCorpus: normalizePreachingEvidenceCorpus(profile.evidenceCorpus)
  };
...
function buildDefaultPreachingProfile(nowIso = "") {
  return {
    ...
    growthGoals: [],
    observations: [],
    evidenceCorpus: normalizePreachingEvidenceCorpus(),
    reviewCadenceSermons: 5,
    baselineApprovedAt: "",
    createdAt: nowIso,
    updatedAt: nowIso
  };
...
function buildPreachingProfileResponse(profile = {}) {
  const response = {
    ...
    version: Math.max(Number(profile.version) || (profile.createdAt || profile.updatedAt ? 1 : 0), 0),
    ...
    avoidances: Array.isArray(profile.avoidances) ? profile.avoidances : [],
    contextGuidance: normalizePreachingContextGuidance(profile.contextGuidance),
    growthGoals: normalizePreachingGrowthGoals(profile.growthGoals),
    observations: normalizeProfileObservations(profile.observations),
    evidenceCorpus: normalizePreachingEvidenceCorpus(profile.evidenceCorpus),
    reviewCadenceSermons: Math.min(Math.max(Number(profile.reviewCadenceSermons) || 5, 1), 25),
  ...
  return {
    ...response,
    fingerprint: buildPreachingProfileFingerprint(response)
  };
}
...
function buildSermonSnapshotRecord(sermon = {}, deps = {}, { snapshotType = "manual", reason = "" } = {}) {
...
async function saveSermonSnapshot(sermon = {}, deps = {}, options = {}) {
  const snapshotsCollection = getSermonSnapshotsCollection(deps);
  const { snapshotId, snapshot } = buildSermonSnapshotRecord(sermon, deps, options);
...
async function loadCompleteSeriesCollection(collection) {
  return (await readCompleteQuery(collection)).map(doc => ({ id: doc.id, data: doc.data() || {} }));
}

function validateSermonSeriesRecordSize(record) {
  if (Buffer.byteLength(JSON.stringify(record), "utf8") > 900000) {
    throw createSermonWorkspaceError("Series Hub storage limit reached; preserve further development on the appropriate sermon hub", 413, {}, "sermon_series_storage_limit");
  }
}

function findMatchingSermonSeriesRecord(records = [], input = {}) {
  const requestedSeriesId = normalizeString(input.seriesId);
  const requestedFolderId = normalizeString(input.folderId);
  const requestedSlug = normalizeString(input.seriesSlug);
  const requestedTitle = normalizeString(input.seriesTitle || input.title || input.name);
  const titleSignature = buildFolderNameSignature(requestedTitle);
  const matches = records.filter(({ id, data }) => {
    if (normalizeString(data.folderType || "series") !== "series") return false;
    const detail = buildSermonSeriesSummary(data, id);
    if (requestedFolderId && detail.folderId === requestedFolderId) return true;
    if (requestedSeriesId && detail.seriesId === requestedSeriesId) return true;
    if (requestedSlug && detail.seriesSlug === requestedSlug) return true;
    if (requestedTitle) {
      return detail.seriesTitle.toLowerCase() === requestedTitle.toLowerCase() ||
        (titleSignature && buildFolderNameSignature(detail.seriesTitle) === titleSignature);
    }
    return false;
  });
  if (matches.length > 1) {
    throw createSermonWorkspaceError(
      "Series identity matched multiple series hubs",
      409,
      {
        matches: matches.slice(0, 10).map(({ id, data }) => buildSermonSeriesSummary(data, id))
      },
      "ambiguous_sermon_series_hub"
    );
  }
  return matches[0] || null;
}

async function getRequiredSermonSeriesDoc(input = {}, deps = {}) {
  const foldersCollection = getFoldersCollection(deps);
  const directId = normalizeString(input.folderId || input.seriesId);
  if (directId) {
    const validId = validateDocId(directId, input.folderId ? "folderId" : "seriesId");
    const directDoc = await foldersCollection.doc(validId).get();
    if (directDoc.exists && normalizeString(directDoc.data()?.folderType || "series") === "series") {
      return { folderId: validId, docRef: foldersCollection.doc(validId), data: directDoc.data() || {} };
    }
  }
  const match = findMatchingSermonSeriesRecord(await loadCompleteSeriesCollection(foldersCollection), input);
  if (!match) {
    throw createSermonWorkspaceError(
      "Sermon series hub not found",
      404,
      {
      ...
      "sermon_series_hub_not_found"
    );
  }
  return {
    folderId: match.data.folderId || match.id,
    docRef: foldersCollection.doc(match.id),
    data: match.data
  };
}

async function resolveSeriesAwareFolderId(input = {}, series = {}, deps = {}) {
  const explicitFolderId = normalizeString(input.folderId);
  if (explicitFolderId) return assertSermonFolderExists(explicitFolderId, deps);
  if (!series.seriesId && !series.seriesSlug && !series.seriesTitle) return "";
  const match = findMatchingSermonSeriesRecord(
    await loadCompleteSeriesCollection(getFoldersCollection(deps)),
    series
  );
  return match ? normalizeString(match.data.folderId || match.id) : "";
}

function buildSermonSeriesRecord(input = {}, deps = {}, fallback = {}) {
  const title = normalizeString(input.seriesTitle || input.title || fallback.seriesTitle || fallback.name);
  if (!title) {
    throw createSermonWorkspaceError(
      "Missing sermon series title",
      400,
      {},
      "missing_sermon_series_title"
    );
  }
  const seriesSlug = normalizeString(input.seriesSlug || fallback.seriesSlug) || normalizeSeriesSlug(title);
  const seriesId = normalizeString(input.seriesId || fallback.seriesId) || `series-${seriesSlug}`;
  validateDocId(seriesId, "seriesId");
  const sourceIdea = typeof input.sourceIdea === "string"
    ? input.sourceIdea
    : typeof fallback.sourceIdea === "string"
      ? fallback.sourceIdea
      : "";
  const seriesStatus = normalizeEnum(
    input.status || input.seriesStatus,
    SERMON_SERIES_STATUSES,
    Object.keys(fallback).length > 0 ? inferSermonSeriesStatus(fallback) : "exploring",
    "sermon_series_status"
  );
  const preachingApproach = normalizeEnum(
    input.preachingApproach,
    SERMON_SERIES_PREACHING_APPROACHES,
    normalizeString(fallback.preachingApproach),
    "sermon_series_preaching_approach"
  );
  const nowIso = getNowIso(deps);
  const record = {
    ...clone(fallback),
    folderId: normalizeString(input.folderId || fallback.folderId || seriesId),
    folderType: "series",
    name: title,
    seriesId,
    seriesTitle: title,
    seriesSlug,
    version: Number.isInteger(Number(fallback.version)) ? Number(fallback.version) : 1,
    seriesStatus,
    status: mapSermonSeriesStatusToFolderStatus(seriesStatus),
    ...
    pastoralBurden: Object.prototype.hasOwnProperty.call(input, "pastoralBurden")
      ? normalizeString(input.pastoralBurden)
      : normalizeString(fallback.pastoralBurden),
    intendedResponse: Object.prototype.hasOwnProperty.call(input, "intendedResponse")
      ? normalizeString(input.intendedResponse)
      : normalizeString(fallback.intendedResponse),
    preachingApproach,
    preachingApproachNotes: Object.prototype.hasOwnProperty.call(input, "preachingApproachNotes")
      ? normalizeString(input.preachingApproachNotes)
      : normalizeString(fallback.preachingApproachNotes),
    scope: Object.prototype.hasOwnProperty.call(input, "scope")
      ? normalizeString(input.scope)
      : normalizeString(fallback.scope),
    boundaries: Object.prototype.hasOwnProperty.call(input, "boundaries")
      ? normalizeStringArray(input.boundaries)
      : normalizeStringArray(fallback.boundaries),
    anchorScripture: Object.prototype.hasOwnProperty.call(input, "anchorScripture")
      ? normalizeString(input.anchorScripture)
      : normalizeString(fallback.anchorScripture || fallback.scriptureScope),
    scriptureScope: Object.prototype.hasOwnProperty.call(input, "anchorScripture")
      ? normalizeString(input.anchorScripture)
      : normalizeString(fallback.scriptureScope || fallback.anchorScripture),
    audience: Object.prototype.hasOwnProperty.call(input, "audience")
      ? normalizeString(input.audience)
      : normalizeString(fallback.audience),
    service: Object.prototype.hasOwnProperty.call(input, "service")
      ? normalizeString(input.service)
      : normalizeString(fallback.service),
    targetStartDate: Object.prototype.hasOwnProperty.call(input, "targetStartDate")
      ? normalizeOptionalDate(input.targetStartDate, "seriesTargetStartDate")
      : normalizeString(fallback.targetStartDate),
    targetEndDate: Object.prototype.hasOwnProperty.call(input, "targetEndDate")
      ? normalizeOptionalDate(input.targetEndDate, "seriesTargetEndDate")
      : normalizeString(fallback.targetEndDate),
    messageMap: Object.prototype.hasOwnProperty.call(input, "messageMap")
      ? normalizeSermonSeriesMessageMap(input.messageMap, seriesId)
      : normalizeSermonSeriesMessageMap(fallback.messageMap || [], seriesId),
    tags: Object.prototype.hasOwnProperty.call(input, "tags")
      ? normalizeTags(input.tags)
      : normalizeTags(fallback.tags),
    sourceIdea,
    sourceIdeaSha256: sourceIdea ? createHash("sha256").update(sourceIdea).digest("hex") : "",
    sourceRefs: Object.prototype.hasOwnProperty.call(input, "sourceRefs")
      ? normalizeSourceRefs(input.sourceRefs)
      : normalizeSourceRefs(fallback.sourceRefs),
    notes: Object.prototype.hasOwnProperty.call(input, "notes")
      ? normalizeString(input.notes)
      : normalizeString(fallback.notes),
    developmentTurns: Array.isArray(fallback.developmentTurns)
      ? fallback.developmentTurns.filter(isPlainObject).map(clone)
      : [],
    createdAt: normalizeString(fallback.createdAt) || nowIso,
    updatedAt: nowIso
  };
  record.searchText = buildSermonSeriesSearchText(record);
  return record;
}

async function createSermonSeries(input = {}, deps = {}) {
  const foldersCollection = getFoldersCollection(deps);
  const title = normalizeString(input.seriesTitle || input.title);
  if (!title) {
    throw createSermonWorkspaceError("Missing sermon series title", 400, {}, "missing_sermon_series_title");
  }
  const requestedSlug = normalizeString(input.seriesSlug) || normalizeSeriesSlug(title);
  const requestedSeriesId = normalizeString(input.seriesId) || `series-${requestedSlug}`;
  const records = await loadCompleteSeriesCollection(foldersCollection);
  const matching = findMatchingSermonSeriesRecord(records, {
    ...input,
  ...
  if (matching) {
    const existing = buildSermonSeriesDetail(matching.data, matching.id);
    if (existing.version > 0 && normalizeString(matching.data.seriesId)) {
      return { action: "existing", series: existing };
    }
    const adopted = buildSermonSeriesRecord({ ...input, folderId: matching.id }, deps, matching.data);
    adopted.version = 1;
    await validateSermonSeriesMessageLinks(adopted.messageMap, adopted.seriesId, deps);
    await commitSermonSeriesRecord({ docRef: foldersCollection.doc(matching.id), folderId: matching.id, data: matching.data }, adopted, deps);
    return { action: "adopted_legacy_series_folder", series: buildSermonSeriesDetail(adopted, matching.id) };
  }

  const folderId = normalizeString(input.folderId)
    ? validateDocId(input.folderId, "folderId")
    : validateDocId(requestedSeriesId, "seriesId");
  const docRef = foldersCollection.doc(folderId);
  const existingDoc = await docRef.get();
  if (existingDoc.exists) {
    throw createSermonWorkspaceError(
      "A non-series folder already uses the requested series hub ID",
      409,
      { folderId },
      "sermon_series_hub_id_conflict"
    );
  }
  const record = buildSermonSeriesRecord({
    ...input,
    folderId,
    seriesId: requestedSeriesId,
    seriesSlug: requestedSlug,
  ...
  record.version = 1;
  await validateSermonSeriesMessageLinks(record.messageMap, record.seriesId, deps);
  validateSermonSeriesRecordSize(record);
  await docRef.create(record);
  return { action: "created", series: buildSermonSeriesDetail(record, folderId) };
}

async function listSermonSeries(input = {}, deps = {}) {
  const status = normalizeString(input.status);
  if (status && !SERMON_SERIES_STATUSES.includes(status)) {
    throw createSermonWorkspaceError(
      "Invalid sermon series status",
      400,
      { status, allowedValues: SERMON_SERIES_STATUSES },
      "invalid_sermon_series_status"
    );
  }
  const query = normalizeString(input.query).toLowerCase();
  const limit = normalizeLimit(input.limit);
  const series = (await loadCompleteSeriesCollection(getFoldersCollection(deps)))
    .filter(({ data }) => normalizeString(data.folderType || "series") === "series")
    .filter(({ id, data }) => !query ||
      (normalizeString(data.searchText) || buildSermonSeriesSearchText({ ...data, folderId: id })).includes(query))
    .map(({ id, data }) => buildSermonSeriesSummary(data, id))
...
function reconcileSermonSeriesMessageMap(series = {}, sermons = []) {
  const claimedSermonIds = new Set();
  const messageMap = series.messageMap.map((item) => {
    const exact = item.sermonId
      ? sermons.find((sermon) => sermon.sermonId === item.sermonId)
      : null;
    const numbered = exact || sermons.find((sermon) =>
      Number(sermon.seriesNumber) === item.seriesNumber && !claimedSermonIds.has(sermon.sermonId));
    if (numbered) claimedSermonIds.add(numbered.sermonId);
    const drift = [];
    if (numbered && item.title && numbered.title && item.title !== numbered.title) drift.push("title");
    if (
      numbered && item.scriptureText && numbered.scriptureText &&
      item.scriptureText.toLowerCase() !== numbered.scriptureText.toLowerCase()
    ) drift.push("scriptureText");
    return {
      ...item,
      linked: Boolean(numbered),
      sermon: numbered || null,
      drift
    };
  });
  return {
    messageMap,
    linkedMessageCount: messageMap.filter((item) => item.linked).length,
    unlinkedMessageCount: messageMap.filter((item) => !item.linked).length,
    unmappedSermons: sermons.filter((sermon) => !claimedSermonIds.has(sermon.sermonId)),
    driftItems: messageMap.filter((item) => item.drift.length > 0).map((item) => ({
...
async function getSermonSeries(input = {}, deps = {}) {
  const record = await getRequiredSermonSeriesDoc(input, deps);
  const series = buildSermonSeriesDetail(record.data, record.folderId);
  const sermonRecords = await loadCompleteSeriesCollection(getSermonsCollection(deps));
  const sermons = sermonRecords
    .filter(({ data }) =>
      normalizeString(data.seriesId) === series.seriesId ||
      (!normalizeString(data.seriesId) && normalizeString(data.folderId) === series.folderId))
    .map(({ id, data }) => buildSermonListSummary(data, id))
    .sort((left, right) =>
      (Number(left.seriesNumber) || Number.MAX_SAFE_INTEGER) -
  ...
  return {
    ...
    reconciliation: reconcileSermonSeriesMessageMap(series, sermons)
  };
}

async function validateSermonSeriesMessageLinks(messageMap = [], seriesId, deps = {}) {
  const linkedItems = messageMap.filter((item) => item.sermonId);
  for (const item of linkedItems) {
    const doc = await getSermonsCollection(deps).doc(item.sermonId).get();
    if (!doc.exists) {
      throw createSermonWorkspaceError(
        "A series message-map item links to a missing sermon",
        400,
        { mapItemId: item.mapItemId, sermonId: item.sermonId },
        "sermon_series_message_sermon_not_found"
      );
    }
    const existingSeriesId = normalizeString(doc.data()?.seriesId);
    if (existingSeriesId && existingSeriesId !== seriesId) {
      throw createSermonWorkspaceError(
        "A linked sermon belongs to a different series",
        409,
        { mapItemId: item.mapItemId, sermonId: item.sermonId, seriesId, existingSeriesId },
        "sermon_series_message_sermon_conflict"
      );
    }
  }
}

function requireExpectedSermonSeriesVersion(input = {}, currentVersion) {
  const expectedVersion = Number(input.expectedVersion);
  if (!Number.isInteger(expectedVersion) || expectedVersion < 0) {
    throw createSermonWorkspaceError(
      "Sermon series update requires expectedVersion",
      400,
      { currentVersion },
      "missing_sermon_series_expected_version"
    );
  }
  if (expectedVersion !== currentVersion) {
    throw createSermonWorkspaceError(
      "Sermon series hub changed since it was read",
      409,
      { expectedVersion, currentVersion },
      "stale_sermon_series_version"
    );
  }
}

async function commitSermonSeriesRecord(record, next, deps) {
  validateSermonSeriesRecordSize(next);
  return runDanFirestoreTransaction(deps, getFoldersCollection(deps), async transaction => {
    const snapshot = await transaction.get(record.docRef);
    if (!snapshot.exists) throw createSermonWorkspaceError("Sermon series hub not found", 404, {}, "sermon_series_hub_not_found");
    const fresh = snapshot.data();
    const expected = buildSermonSeriesSummary(record.data, record.folderId).version;
    requireExpectedSermonSeriesVersion({ expectedVersion: expected }, buildSermonSeriesSummary(fresh, record.folderId).version);
    if (fresh.updatedAt !== record.data.updatedAt) throw createSermonWorkspaceError("Series folder changed during editing", 409, {}, "stale_sermon_series_version");
    transaction.set(record.docRef, next);
  });
}

async function updateSermonSeries(input = {}, deps = {}) {
  const record = await getRequiredSermonSeriesDoc(input, deps);
  const current = buildSermonSeriesDetail(record.data, record.folderId);
  requireExpectedSermonSeriesVersion(input, current.version);
  const changes = isPlainObject(input.changes) ? input.changes : {};
  if (Object.prototype.hasOwnProperty.call(changes, "seriesId") &&
      normalizeString(changes.seriesId) !== current.seriesId) {
    throw createSermonWorkspaceError(
      "A sermon series ID is stable and cannot be changed",
      409,
      { seriesId: current.seriesId },
      "sermon_series_id_immutable"
    );
  }
  if (Object.prototype.hasOwnProperty.call(changes, "seriesSlug") &&
      normalizeString(changes.seriesSlug) !== current.seriesSlug) {
    throw createSermonWorkspaceError(
      "A sermon series slug is stable and cannot be changed",
      409,
      { seriesSlug: current.seriesSlug },
      "sermon_series_slug_immutable"
    );
  }
  if (
    Object.prototype.hasOwnProperty.call(changes, "sourceIdea") &&
    current.sourceIdea &&
    changes.sourceIdea !== current.sourceIdea
  ) {
    throw createSermonWorkspaceError(
      "The exact source idea is immutable; append a development turn instead",
      409,
      { seriesId: current.seriesId },
      "sermon_series_source_idea_immutable"
    );
  }
  const next = buildSermonSeriesRecord({
    ...changes,
    folderId: current.folderId,
    seriesId: current.seriesId,
    seriesSlug: current.seriesSlug,
    seriesTitle: Object.prototype.hasOwnProperty.call(changes, "seriesTitle")
      ? changes.seriesTitle
  ...
  if (Object.prototype.hasOwnProperty.call(changes, "messageMap")) {
    await validateSermonSeriesMessageLinks(next.messageMap, current.seriesId, deps);
  }
  await commitSermonSeriesRecord(record, next, deps);
  return getSermonSeries({ seriesId: current.seriesId }, deps);
}

async function appendSermonSeriesDevelopmentTurn(input = {}, deps = {}) {
  const record = await getRequiredSermonSeriesDoc(input, deps);
  const current = buildSermonSeriesDetail(record.data, record.folderId);
  requireExpectedSermonSeriesVersion(input, current.version);
  ...
  if (!transcript.trim()) {
    throw createSermonWorkspaceError(
      "Series development turn requires the complete transcript",
      400,
      {},
      "missing_sermon_series_development_transcript"
    );
  }
  if (transcript.length > MAX_SERMON_DEVELOPMENT_TURN_LENGTH) {
    throw createSermonWorkspaceError(
      "Series development turn is too large",
      413,
      { maximumLength: MAX_SERMON_DEVELOPMENT_TURN_LENGTH },
      "sermon_series_development_turn_too_large"
    );
  }
  const existingTurns = current.developmentTurns;
  if (existingTurns.length >= MAX_SERMON_SERIES_DEVELOPMENT_TURNS) {
    throw createSermonWorkspaceError(
      "Series hub has reached its development-turn limit",
      409,
      { maximumTurns: MAX_SERMON_SERIES_DEVELOPMENT_TURNS },
      "sermon_series_development_turn_limit"
    );
  }
  const speaker = normalizeEnum(
    ...
    "dan",
    "sermon_series_development_speaker"
  );
  const sequence = existingTurns.reduce((maximum, turn) => Math.max(maximum, Number(turn.sequence) || 0), 0) + 1;
  ...
  const turnId = normalizeString(input.turnId)
    ...
    : `series-turn-${createHash("sha256")
        .update(`${current.seriesId}:${sequence}:${speaker}:${transcriptSha256}`)
        .digest("hex").slice(0, 24)}`;
  const existingTurn = existingTurns.find((turn) => normalizeString(turn.turnId) === turnId);
  if (existingTurn) {
    if (
      normalizeString(existingTurn.transcriptSha256) !== transcriptSha256 ||
      normalizeString(existingTurn.speaker) !== speaker
    ) {
      throw createSermonWorkspaceError(
        "A different series development turn already uses this ID",
        409,
        { turnId },
        "sermon_series_development_turn_conflict"
      );
    }
    return { action: "replayed", series: current, turn: existingTurn };
  }
  const totalLength = existingTurns.reduce((total, turn) => total + String(turn.transcript || "").length, 0) + transcript.length;
  if (totalLength > MAX_SERMON_SERIES_DEVELOPMENT_TEXT_LENGTH) {
    throw createSermonWorkspaceError(
      "Series development transcript storage limit reached",
      413,
      { maximumLength: MAX_SERMON_SERIES_DEVELOPMENT_TEXT_LENGTH },
      "sermon_series_development_text_limit"
    );
  }
  const nowIso = getNowIso(deps);
  const turn = {
    ...
    transcript,
    transcriptSha256,
    exactWording: input.exactWording === true || speaker === "dan",
    sourceMode: normalizeString(input.sourceMode) || "chat",
    createdAt: nowIso,
    updatedAt: nowIso
  };
  const next = {
    ...
    developmentTurns: existingTurns.concat(turn),
    lastDevelopedAt: nowIso,
    updatedAt: nowIso
  };
  next.searchText = buildSermonSeriesSearchText(next);
  await commitSermonSeriesRecord(record, next, deps);
  return {
    action: "created",
    series: buildSermonSeriesDetail(next, current.folderId),
    turn
  };
}
...
async function createSermon(input = {}, deps = {}) {
  ...
  const folderId = await resolveSeriesAwareFolderId(input, series, deps);
...
async function reviewSermonSeriesProgression(input = {}, deps = {}) {
  ...
  let seriesHub = null;
  try {
    const hubRecord = await getRequiredSermonSeriesDoc({ seriesId, seriesSlug, seriesTitle }, deps);
    seriesHub = buildSermonSeriesDetail(hubRecord.data, hubRecord.folderId);
  } catch (error) {
    if (error?.code !== "sermon_series_hub_not_found") throw error;
  }
  const messageMapReconciliation = seriesHub
    ? reconcileSermonSeriesMessageMap(seriesHub, sermons)
    : null;
  if (!seriesHub) {
    recommendations.push("Create a Series Hub so the pastoral burden, preaching approach, boundaries, and provisional message map are durable.");
  }
...
async function upsertImportedSermonOccasions(
  sermonId,
  occasionInputs = [],
  deps = {},
  options = {}
  ...
  const emptyReceipt = {
    ...
    unchanged: [],
    conflicts: [],
    counts: { created: 0, updated: 0, unchanged: 0, conflicts: 0 }
  };
  if (!Array.isArray(occasionInputs) || occasionInputs.length === 0) {
    return detailed ? emptyReceipt : [];
  }

  const occasionsCollection = getSermonOccasionsCollection(deps);
  const existingRecords = await loadSermonOccasionRecords(occasionsCollection, {
    sermonId,
    maxDocs: 20000
  });
  const workingRecords = existingRecords.map(({ id, data }) => ({
    id,
    data: { ...(data || {}), occasionId: normalizeString(data?.occasionId || id), sermonId }
  }));
  const receipt = clone(emptyReceipt);

  const sameIdentity = (left = {}, right = {}) =>
    normalizeString(left.date) === normalizeString(right.date) &&
    normalizeString(left.time) === normalizeString(right.time) &&
    normalizeString(left.venue).toLowerCase() === normalizeString(right.venue).toLowerCase() &&
    normalizeString(left.service).toLowerCase() === normalizeString(right.service).toLowerCase();
  const compatibleForEnrichment = (current = {}, incoming = {}) =>
    ["time", "venue", "service"].every((field) => {
      const currentValue = normalizeString(current[field]).toLowerCase();
      const incomingValue = normalizeString(incoming[field]).toLowerCase();
      return !currentValue || !incomingValue || currentValue === incomingValue;
    });
  const summarizeConflictCandidate = ({ id, data }) => ({
  ...
  for (const input of occasionInputs) {
    ...
    const normalized = normalizeSermonOccasion({ ...input, sermonId });
    const nowIso = getNowIso(deps);
    const exactMatches = workingRecords.filter(({ data }) => sameIdentity(data, normalized));

    if (exactMatches.length === 1) {
      const match = exactMatches[0];
      const summary = buildSermonOccasionSummary(match.data, match.id);
      receipt.occasions.push(summary);
      receipt.unchanged.push(summary);
      if (
        normalizeString(match.data.status) &&
        normalizeString(normalized.status) &&
        normalizeString(match.data.status) !== normalizeString(normalized.status)
      ) {
        receipt.conflicts.push({
          type: "occasion_status_conflict",
          date: normalized.date,
          incoming: buildSermonOccasionSummary(normalized),
          candidates: [summarizeConflictCandidate(match)]
        });
      }
      continue;
    }
    ...
    if (exactMatches.length > 1) {
      receipt.conflicts.push({
        type: "duplicate_exact_occasions",
        date: normalized.date,
        incoming: buildSermonOccasionSummary(normalized),
        candidates: exactMatches.map(summarizeConflictCandidate)
      });
      continue;
    }

    const sameDate = workingRecords.filter(({ data }) =>
      normalizeString(data.date) === normalizeString(normalized.date));
    const compatible = sameDate.filter(({ data }) => compatibleForEnrichment(data, normalized));
    ...
    if (compatible.length === 1) {
      ...
      const next = { ...current };
      const changedFields = [];
      for (const field of ["time", "venue", "service"]) {
        if (!normalizeString(current[field]) && normalizeString(normalized[field])) {
      ...
      if (
        ...
        normalizeString(normalized.status) &&
        normalizeString(current.status) !== normalizeString(normalized.status)
      ) {
        receipt.conflicts.push({
          type: "occasion_status_conflict",
          date: normalized.date,
      ...
      if (changedFields.length === 0) {
        const summary = buildSermonOccasionSummary(current, match.id);
        receipt.occasions.push(summary);
        receipt.unchanged.push(summary);
        continue;
      }

      next.occasionId = normalizeString(current.occasionId || match.id);
      next.sermonId = sermonId;
      next.updatedAt = nowIso;
      next.sortKey = buildOccasionSortKey(next);
      next.searchText = buildSermonOccasionSearchText(next);
      await occasionsCollection.doc(match.id).set(next);
      match.data = next;
      const summary = {
        ...buildSermonOccasionSummary(next, match.id),
        changedFields
      };
      receipt.occasions.push(summary);
      receipt.updated.push(summary);
      continue;
    }

    if (sameDate.length > 0) {
      receipt.conflicts.push({
        type: compatible.length > 1
          ? "ambiguous_same_date_occasion"
          : "conflicting_same_date_occasion",
        date: normalized.date,
        incoming: buildSermonOccasionSummary(normalized),
        candidates: (compatible.length > 1 ? compatible : sameDate).map(summarizeConflictCandidate)
      });
      continue;
    }
    ...
    occasion.searchText = buildSermonOccasionSearchText(occasion);
    await occasionsCollection.doc(occasionId).create(occasion);
    workingRecords.push({ id: occasionId, data: occasion });
    const summary = buildSermonOccasionSummary(occasion, occasionId);
    receipt.occasions.push(summary);
    receipt.created.push(summary);
  }
  ...
  if (receipt.created.length > 0 || receipt.updated.length > 0) {
    await syncSermonOccasionCompatibilityFields(sermonId, deps);
  }
  return detailed ? receipt : receipt.occasions;
}
...
async function updateSermon(input = {}, deps = {}) {
  ...
  if (
    ...
    if (!normalizeString(nextSermon.folderId)) {
      nextSermon.folderId = await resolveSeriesAwareFolderId({}, series, deps);
    }
  }
...
async function importSermonMaterial(input = {}, deps = {}) {
  ...
  const explicitImportStatus = normalizeString(input.status);
  const isPreachedTranscriptImport = sourceType === "preached_transcript";
  const importedOccasionInputs = Array.isArray(input.occasions)
    ? input.occasions.filter(isPlainObject).map((occasion) => ({
      ...
      status: normalizeString(occasion.status) ||
        (explicitImportStatus === "preached" || normalizeString(input.preachedDate) || isPreachedTranscriptImport
          ? "preached"
          : "planned")
    }))
    : [];
  ...
  if (shouldSaveSource && sourceId) {
    ...
    if (existingSourceDoc.exists) {
      ...
      if (input.refreshExistingSource === true) {
        const currentSermon = { ...(existingSermonDoc.data() || {}), sermonId: existingSermonId };
        const nextSermon = clone(currentSermon);
        const canonicalChanges = [];
        const conflicts = [];

        if (title && !normalizeString(nextSermon.title)) {
          nextSermon.title = title;
          canonicalChanges.push("title");
        }
        if (scriptureText && !normalizeString(nextSermon.scriptureText)) {
          nextSermon.scriptureText = scriptureText;
          canonicalChanges.push("scriptureText");
        } else if (
          scriptureText &&
          normalizeString(nextSermon.scriptureText).toLowerCase() !== scriptureText.toLowerCase()
        ) {
          conflicts.push({
            field: "scriptureText",
            sermonWorkspace: normalizeString(nextSermon.scriptureText),
            logos: scriptureText
          });
        }
        if (input.preachedDate && !normalizeString(nextSermon.preachedDate)) {
          nextSermon.preachedDate = normalizeOptionalDate(input.preachedDate, "preachedDate");
          canonicalChanges.push("preachedDate");
        }

        const currentSeries = buildSeriesMetadata({}, nextSermon);
        if (series.seriesTitle && !currentSeries.seriesTitle) {
          nextSermon.seriesId = series.seriesId;
          nextSermon.seriesTitle = series.seriesTitle;
          nextSermon.seriesSlug = series.seriesSlug;
          nextSermon.seriesNumber = series.seriesNumber;
          nextSermon.series = series.series;
          canonicalChanges.push("series");
        } else if (
          series.seriesTitle &&
          currentSeries.seriesTitle.toLowerCase() === series.seriesTitle.toLowerCase() &&
          !currentSeries.seriesNumber &&
          series.seriesNumber
        ) {
          const numberedSeries = buildSeriesMetadata({
            ...currentSeries,
            seriesNumber: series.seriesNumber
          });
          nextSermon.seriesId = numberedSeries.seriesId;
          nextSermon.seriesTitle = numberedSeries.seriesTitle;
          nextSermon.seriesSlug = numberedSeries.seriesSlug;
          nextSermon.seriesNumber = numberedSeries.seriesNumber;
          nextSermon.series = numberedSeries.series;
          canonicalChanges.push("seriesNumber");
        } else if (
          series.seriesTitle &&
          currentSeries.seriesTitle.toLowerCase() !== series.seriesTitle.toLowerCase()
        ) {
          conflicts.push({
            field: "seriesTitle",
            sermonWorkspace: currentSeries.seriesTitle,
            logos: series.seriesTitle
          });
        }

        const mergedTags = mergeTagsCaseInsensitive(nextSermon.tags, tags);
        const addedTags = mergedTags.filter((tag) =>
          !normalizeTags(nextSermon.tags).some((current) => current.toLowerCase() === tag.toLowerCase()));
        if (addedTags.length > 0) {
          nextSermon.tags = mergedTags;
          canonicalChanges.push("tags");
        }

        let snapshot = null;
        let snapshotWrite = null;
        if (canonicalChanges.length > 0) {
          snapshotWrite = buildSermonSnapshotRecord(currentSermon, deps, {
            snapshotType: "before_import_refresh",
            reason: normalizeString(input.snapshotReason) ||
              `Before refreshing canonical metadata from ${sourceLabel}`
          });
          snapshot = buildSermonSnapshotSummary(
            snapshotWrite.snapshot,
            snapshotWrite.snapshotId
          );
          nextSermon.updatedAt = nowIso;
          nextSermon.searchText = buildSermonSearchText(nextSermon);
        }

        const nextSource = clone(existingSource);
        const mergedSourceTags = mergeTagsCaseInsensitive(existingSource.tags, tags);
        const mergedRefs = mergeSourceRefs(existingSource.sourceRefs, sourceRefs);
        const nextSummary = importedSummary || normalizeString(existingSource.summary);
        const nextMaterial = normalizeString(existingSource.material) || importedMaterial;
        const sourceSeries = buildSeriesMetadata(
          series.seriesTitle ? series : {},
          existingSource.seriesTitle ? existingSource : nextSermon
        );
        Object.assign(nextSource, {
          folderId: normalizeString(existingSource.folderId || nextSermon.folderId),
          seriesId: sourceSeries.seriesId,
          seriesTitle: sourceSeries.seriesTitle,
          seriesSlug: sourceSeries.seriesSlug,
          seriesNumber: sourceSeries.seriesNumber,
          series: sourceSeries.series,
          tags: mergedSourceTags,
          sourceLabel: normalizeString(existingSource.sourceLabel) || sourceLabel,
          summary: nextSummary,
        ...
        const sourceChanged = [
          ...
          "material",
          "sourceRefs"
        ].some((field) => JSON.stringify(existingSource[field] ?? null) !== JSON.stringify(nextSource[field] ?? null));
        if (sourceChanged) {
          nextSource.updatedAt = nowIso;
          nextSource.searchText = buildSermonSourceSearchText(nextSource);
        }

        if (canonicalChanges.length > 0 || sourceChanged) {
          assertExpectedImportRefreshState({
            sermonId: existingSermonId,
            sourceId,
            currentSermon,
            currentSource: existingSource,
            expectedSermonUpdatedAt: input.expectedSermonUpdatedAt,
            expectedSourceUpdatedAt: input.expectedSourceUpdatedAt
          });
          await commitSermonImportRefresh({
            sermonId: existingSermonId,
            sourceId,
            currentSermon,
            currentSource: existingSource,
            nextSermon,
            nextSource,
            sermonChanged: canonicalChanges.length > 0,
            sourceChanged,
            snapshotWrite
          }, deps);
        }

        const occasionChanges = await upsertImportedSermonOccasions(
          existingSermonId,
          importedOccasionInputs,
          deps,
          { detailed: true }
        );
        const occasionWriteCount =
          occasionChanges.counts.created + occasionChanges.counts.updated;
        const combinedConflicts = conflicts.concat(occasionChanges.conflicts);
        const sermon = occasionWriteCount > 0 || canonicalChanges.length > 0
          ? (await getSermon({ sermonId: existingSermonId }, deps)).sermon
          : buildSermonDetail(currentSermon, existingSermonId);
        const changed = canonicalChanges.length > 0 || sourceChanged || occasionWriteCount > 0;

        return {
          action: changed
            ? (combinedConflicts.length > 0
              ? "refreshed_existing_source_with_conflicts"
              : "refreshed_existing_source")
            : (combinedConflicts.length > 0 ? "review_required" : "skipped_existing_source"),
          sermon,
          importedNoteCount: 0,
          materialSaved: !normalizeString(existingSource.material) && Boolean(importedMaterial),
          sourceSaved: sourceChanged,
          source: buildSermonSourceDetail(sourceChanged ? nextSource : existingSource, sourceId),
          occasions: occasionChanges.occasions,
          occasionChanges,
          canonicalChanges,
          addedTags,
      ...
      return {
        action: occasionWriteCount > 0
          ? "updated_existing_occasions"
          : (occasionChanges.conflicts.length > 0 ? "review_required" : "skipped_existing_source"),
        sermon: occasionWriteCount > 0
          ? (await getSermon({ sermonId: existingSermonId }, deps)).sermon
        ...
        occasions: occasionChanges.occasions,
        occasionChanges,
        conflicts: occasionChanges.conflicts
      };
    }
  }
  ...
  if (!target) {
    ...
    const sermon = {
      ...
      status: normalizeEnum(
        input.status,
        SERMON_STATUSES,
        isPreachedTranscriptImport ? "preached" : "idea",
        "sermon_status"
      ),
      scriptureText,
      bigIdea: normalizeString(input.bigIdea),
    ...
    const occasionWriteCount =
      occasionChanges.counts.created + occasionChanges.counts.updated;
    const enrichedSermon = occasionWriteCount > 0
  ...
  } else if (isPreachedTranscriptImport && nextSermon.status !== "archived") {
    nextSermon.status = "preached";
  }
...
async function importSermonMaterialBatch(input = {}, deps = {}) {
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    try {
      ...
      const result = {
        ...
        occasionChanges: importResult.occasionChanges || {
          occasions: [],
          created: [],
          updated: [],
          unchanged: [],
        ...
        canonicalChanges: Array.isArray(importResult.canonicalChanges)
          ? importResult.canonicalChanges
          : [],
        addedTags: Array.isArray(importResult.addedTags) ? importResult.addedTags : [],
        conflicts: Array.isArray(importResult.conflicts) ? importResult.conflicts : [],
        snapshot: importResult.snapshot || null,
        importedNoteCount: importResult.importedNoteCount,
        materialSaved: importResult.materialSaved === true,
  ...
  const actionCounts = {};
  const canonicalChangeCounts = {};
  let addedTagCount = 0;
  let conflictCount = 0;
  const occasionCounts = { created: 0, updated: 0, unchanged: 0, conflicts: 0 };
  for (const result of results) {
    actionCounts[result.action] = (actionCounts[result.action] || 0) + 1;
    for (const field of result.canonicalChanges) {
      canonicalChangeCounts[field] = (canonicalChangeCounts[field] || 0) + 1;
    }
    addedTagCount += result.addedTags.length;
    conflictCount += result.conflicts.length;
    for (const field of Object.keys(occasionCounts)) {
      occasionCounts[field] += Number(result.occasionChanges?.counts?.[field] || 0);
    }
  }
  ...
  return {
    ...
    receiptSummary: {
      actionCounts,
      canonicalChangeCounts,
      addedTagCount,
      conflictCount,
      occasionCounts,
      snapshotCount: results.filter((result) => result.snapshot).length
    },
    results,
    errors
  };
...
async function updatePreachingProfile(input = {}, deps = {}) {
  ...
  const currentVersion = Math.max(
    Number(existingProfile.version) || (doc.exists ? 1 : 0),
    0
  );
  if (
    ...
    throw createSermonWorkspaceError(
      "The preaching profile changed after it was read",
      409,
      {
        profileId,
        expectedVersion: Number(input.expectedVersion),
        currentVersion,
        nextAction: "Retrieve the current preaching profile and review the proposed changes again."
      },
      "stale_preaching_profile"
    );
  }
  const changes = isPlainObject(input.changes) ? input.changes : input;
  ...
  for (const field of ["tone", "strengths", "recurringPatterns", "cautions", "avoidances"]) {
    if (Object.prototype.hasOwnProperty.call(changes, field)) {
      nextProfile[field] = normalizeStringArray(changes[field]);
    }
  }

  if (Object.prototype.hasOwnProperty.call(changes, "contextGuidance")) {
    nextProfile.contextGuidance = normalizePreachingContextGuidance(changes.contextGuidance);
  }

  if (Object.prototype.hasOwnProperty.call(changes, "growthGoals")) {
    nextProfile.growthGoals = normalizePreachingGrowthGoals(changes.growthGoals);
  }

  if (Object.prototype.hasOwnProperty.call(changes, "evidenceCorpus")) {
    nextProfile.evidenceCorpus = normalizePreachingEvidenceCorpus(changes.evidenceCorpus);
  }

  if (Object.prototype.hasOwnProperty.call(changes, "reviewCadenceSermons")) {
    nextProfile.reviewCadenceSermons = Math.min(
      Math.max(Number(changes.reviewCadenceSermons) || 5, 1),
      25
    );
  }

  if (Object.prototype.hasOwnProperty.call(changes, "baselineApprovedAt")) {
    nextProfile.baselineApprovedAt = normalizeString(changes.baselineApprovedAt);
  }
  ...
  nextProfile.version = currentVersion + 1;
...
async function createPreachingAnalysis(input = {}, deps = {}) {
  ...
  if (input.applyProfileCandidates === true && profileCandidates.length > 0) {
    const currentProfile = await getPreachingProfile({}, deps);
    const profileResult = await updatePreachingProfile(
      {
        expectedVersion: currentProfile.profile.version,
        changes: { observations: profileCandidates }
      },
      deps
    );
