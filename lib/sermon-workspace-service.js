...
async function upsertImportedSermonOccasions(sermonId, occasionInputs = [], deps = {}) {
  ...
  for (const input of occasionInputs) {
    ...
    const unchanged = existing.exists && [
      ...
      "service",
      "notes"
    ].every((field) => normalizeString(existingData[field]) === normalizeString(occasion[field]));
    if (!unchanged) await docRef.set(occasion);
    ...
    if (existingSourceDoc.exists) {
      ...
      if (input.refreshExistingSource === true) {
        ...
        if (canonicalChanges.length > 0) {
          snapshot = await saveSermonSnapshot(currentSermon, deps, {
          ...
          nextSermon.updatedAt = nowIso;
          nextSermon.searchText = buildSermonSearchText(nextSermon);
          await sermonsCollection.doc(existingSermonId).set(nextSermon);
        }
        ...
        if (sourceChanged) {
          ...
          await getSermonSourcesCollection(deps).doc(sourceId).set(nextSource);
        }
        ...
        const sermon = occasions.length > 0 || canonicalChanges.length > 0
          ? (await getSermon({ sermonId: existingSermonId }, deps)).sermon
          : buildSermonDetail(currentSermon, existingSermonId);

        return {
          action: canonicalChanges.length > 0 || sourceChanged
