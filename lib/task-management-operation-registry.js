...
const TASK_MANAGEMENT_OPERATIONS = Object.freeze([
  ...
  defineOperation({
    ...
    optional: ["parentProjectId", "ancestorProjectId", "status", "lifeArea", "department", "teamId", "priority", "targetOnOrBefore", "query", "limit", "cursor"],
    exampleArguments: { status: "active", lifeArea: "work", department: "education", limit: 25 },
    argumentGuidance: "Use parentProjectId for direct children (empty string lists roots), or ancestorProjectId for a whole branch including its root. Follow nextCursor while hasMore is true. Hierarchy does not grant access.",
  ...
  defineOperation({
    ...
    required: ["projectId"],
    optional: ["includeDescendants", "today"],
  ...
  defineOperation({
    ...
    required: ["name"],
    optional: ["projectId", "parentProjectId", "outcome", "lifeArea", "department", "teamId", "status", "priority", "targetDate", "requestedBy", "leadSub", "leadName", "leadEmail", "health", "healthNote", "nextReviewDate", "milestones", "dependencies", "notes", "visibility"],
    ...
    argumentGuidance: "Set parentProjectId to create a child project; omit it for a root. Each project keeps its own lead, dates, status, department and access. Hierarchy does not grant access. Department is optional and must be facsimiles, displays, retail, or education. Every task linked directly to this project inherits this department.",
