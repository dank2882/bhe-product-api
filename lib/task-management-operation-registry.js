...
const maintenanceInbox = require("./maintenance-inbox-service");
...
const TASK_MANAGEMENT_OPERATIONS = Object.freeze([
  defineOperation({ name: "listMaintenanceBoard", mode: "query", required: [], optional: ["includeArchived", "limit", "cursor", "building", "area", "status", "priority", "assignedTo", "unassigned", "query", "reference", "detailLevel"], summary: "Return a ready-to-display working table in chat with ten columns, displayed item numbers, stable references, building/area grouping, counts and filters. Display markdown directly; compact notes by default. Resolve all requested item numbers from the displayed selection mapping before any changes; delete means recoverable archive. Use existing versioned task commands for requested changes and refresh after the batch. Do not export a spreadsheet unless explicitly requested.", handler: maintenance.listMaintenanceBoard }),
  defineOperation({ name: "getMaintenanceAccess", mode: "query", required: [], optional: [], summary: "Read Maintenance managers and branch members.", handler: maintenance.getMaintenanceAccess }),
  defineOperation({ name: "setMaintenanceAccess", mode: "command", required: ["kind", "subject", "role", "expectedVersion"], optional: [], summary: "Manage only Maintenance access. Only the area owner may appoint managers; managers may grant viewer/editor access. Resolve an individual staff subject before granting.", handler: maintenance.setMaintenanceAccess }),
  defineOperation({ name: "listMaintenanceRoutines", mode: "query", required: [], optional: ["includeArchived", "limit", "cursor", "building", "area", "status", "priority", "assignedTo", "unassigned", "query", "reference", "detailLevel"], summary: "Display recurring Maintenance duties as a separate numbered in-chat table with stable references, filters and completion context. Check hasMore. Use detailLevel full and reference for original notes; recordMaintenanceRoutineCompletion records an occurrence without ending the duty.", handler: maintenance.listMaintenanceRoutines }),
  defineOperation({ name: "recordMaintenanceRoutineCompletion", mode: "command", required: ["routineId", "occurrenceKey", "expectedVersion"], optional: ["notes"], summary: "Record one recurring duty occurrence, preserving its ongoing responsibility and completion history. Use a stable date or occurrence key.", handler: maintenance.recordMaintenanceRoutineCompletion }),
  defineOperation({ name: "listMaintenanceRoutineCompletions", mode: "query", required: ["routineId"], optional: [], summary: "Read the completion history of one Maintenance routine.", handler: maintenance.listMaintenanceRoutineCompletions }),
  defineOperation({ name: "listMaintenanceInbox", mode: "query", summary: "Read Maintenance inbox; view:true adds review cards, proposed fields, photo thumbnails and possible existing-task matches. Use tasks_open_maintenance_inbox for clickable approvals. Requires Maintenance manager permission; drafts never send. Approval requires current version and content digest. See getMaintenanceMessagingGuide.", optional: ["view", "proposals", "messageId", "reporterId", "changes", "expectedVersion", "draftId", "contentDigest", "channel", "recipient", "body", "subject", "taskId", "decision", "mediaIds", "mediaId", "cursor", "limit", "requestKey"], handler: maintenanceInbox.listMaintenanceInbox }),
  defineOperation({ name: "getMaintenanceMessage", mode: "query", summary: "Maintenance communications: getMessage. Requires Maintenance manager permission; drafts never send. Approval requires current version and content digest. See getMaintenanceMessagingGuide.", optional: ["messageId", "reporterId", "changes", "expectedVersion", "draftId", "contentDigest", "channel", "recipient", "body", "subject", "taskId", "decision", "mediaIds", "mediaId", "cursor", "limit", "requestKey"], handler: (input, deps) => maintenance.maintenanceMessaging({ ...input, action: "getMessage" }, deps) }),
  defineOperation({ name: "listMaintenanceReporters", mode: "query", summary: "Maintenance communications: listReporters. Requires Maintenance manager permission; drafts never send. Approval requires current version and content digest. See getMaintenanceMessagingGuide.", optional: ["messageId", "reporterId", "changes", "expectedVersion", "draftId", "contentDigest", "channel", "recipient", "body", "subject", "taskId", "decision", "mediaIds", "mediaId", "cursor", "limit", "requestKey"], handler: (input, deps) => maintenance.maintenanceMessaging({ ...input, action: "listReporters" }, deps) }),
  defineOperation({ name: "setMaintenanceReporter", mode: "command", summary: "Maintenance communications: setReporter. Requires Maintenance manager permission; drafts never send. Approval requires current version and content digest. See getMaintenanceMessagingGuide.", optional: ["messageId", "reporterId", "changes", "expectedVersion", "draftId", "contentDigest", "channel", "recipient", "body", "subject", "taskId", "decision", "mediaIds", "mediaId", "cursor", "limit", "requestKey"], handler: (input, deps) => maintenance.maintenanceMessaging({ ...input, action: "setReporter" }, deps) }),
  defineOperation({ name: "reviewMaintenanceMessage", mode: "command", summary: "Review Maintenance request: decision approve saves approval and creates/links one task when title/building/area are known; details completes a saved approval without reapproval. Pass fields:{title,building,area}, current expectedVersion; choose taskId/reference for existing work or confirmNew:true only after explicit distinct-task choice. Do not create a separate task. Legacy link/dismiss supported. Requires Maintenance manager permission; drafts never send. Approval requires current version and content digest. See getMaintenanceMessagingGuide.", optional: ["fields", "confirmNew", "reference", "messageId", "reporterId", "changes", "expectedVersion", "draftId", "contentDigest", "channel", "recipient", "body", "subject", "taskId", "decision", "mediaIds", "mediaId", "cursor", "limit", "requestKey"], handler: maintenanceInbox.reviewMaintenanceMessage }),
  defineOperation({ name: "draftMaintenanceMessage", mode: "command", summary: "Prepare reviewed Maintenance email. template:work_order uses taskId and recipient plus optional recipientName/instructions/accessNotes/mediaIds and required stable requestKey to create a standard Work Order draft. Does not send. Requires Maintenance manager permission; drafts never send. Approval requires current version and content digest. See getMaintenanceMessagingGuide.", optional: ["template", "recipientName", "instructions", "accessNotes", "messageId", "reporterId", "changes", "expectedVersion", "draftId", "contentDigest", "channel", "recipient", "body", "subject", "taskId", "decision", "mediaIds", "mediaId", "cursor", "limit", "requestKey"], handler: maintenanceInbox.draftMaintenanceMessage }),
  defineOperation({ name: "approveMaintenanceMessage", mode: "command", summary: "Maintenance communications: approveMessage. Requires Maintenance manager permission; drafts never send. Approval requires current version and content digest. See getMaintenanceMessagingGuide.", optional: ["messageId", "reporterId", "changes", "expectedVersion", "draftId", "contentDigest", "channel", "recipient", "body", "subject", "taskId", "decision", "mediaIds", "mediaId", "cursor", "limit", "requestKey"], handler: (input, deps) => maintenance.maintenanceMessaging({ ...input, action: "approveMessage" }, deps) }),
  defineOperation({ name: "getMaintenanceOutbox", mode: "query", summary: "Maintenance communications: getOutbox. Requires Maintenance manager permission; drafts never send. Approval requires current version and content digest. See getMaintenanceMessagingGuide.", optional: ["messageId", "reporterId", "changes", "expectedVersion", "draftId", "contentDigest", "channel", "recipient", "body", "subject", "taskId", "decision", "mediaIds", "mediaId", "cursor", "limit", "requestKey"], handler: (input, deps) => maintenance.maintenanceMessaging({ ...input, action: "getOutbox" }, deps) }),
  defineOperation({ name: "getMaintenanceMediaDownload", mode: "query", summary: "Maintenance communications: getMediaDownload. Requires Maintenance manager permission; drafts never send. Approval requires current version and content digest. See getMaintenanceMessagingGuide.", optional: ["messageId", "reporterId", "changes", "expectedVersion", "draftId", "contentDigest", "channel", "recipient", "body", "subject", "taskId", "decision", "mediaIds", "mediaId", "cursor", "limit", "requestKey"], handler: (input, deps) => maintenance.maintenanceMessaging({ ...input, action: "getMediaDownload" }, deps) }),
  defineOperation({ name: "retryMaintenanceMedia", mode: "command", summary: "Retry failed photo processing without resending messages.", required: ["messageId", "expectedVersion"], handler: (input, deps) => maintenance.maintenanceMessaging({ ...input, action: "retryMedia" }, deps) }),
  defineOperation({ name: "getMaintenanceMessagingGuide", mode: "query", summary: "Read exact messaging schemas and workflow guidance.", handler: (input, deps) => maintenance.maintenanceMessaging({ action: "guide" }, deps) }),
  defineOperation({
    name: "listTeams",
    mode: "query",
  ...
  defineOperation({
    ...
    mode: "query",
    summary: "Create a short-lived download link and, for raster photos, an inline image preview for one accessible task or project attachment.",
    required: ["attachmentId"],
    exampleArguments: { attachmentId: "attachment-example" },
    argumentGuidance: "For Maintenance photo previews use tasks_open_maintenance_photos with the task reference. The preview.url supports the embedded viewer or direct opening; also offer download.url. Do not depend on Markdown image embedding. Use returned URLs only before expiration; never store a signed URL on the task.",
  ...
  defineOperation({
    ...
    argumentGuidance: "Maintenance fields require a project under proj-fbc-maintenance: maintenance:{building,area,thirdParty,workflowStatus,cost:{estimate:null|number,actual:null|number,currency:USD,note},targetDateText,sourceStatus,sourcePriority,sourceReferences:[]}. Updating maintenance replaces that object; preserve existing values. workflowStatus: Not Started, Planning, Assessment Needed, Quote Needed, Quote / Schedule, Waiting, In Progress, Completed, Deferred. Completed work requires task status done after explicit approval. A standalone task may optionally use facsimiles, displays, retail, or education. A project task always inherits the project's department, and conflicting input is rejected. Use workOnDate and optional workOnStartTime/workOnEndTime for when the assignee plans to work. dueDate remains a real deadline. scheduledDate and scheduledTime are compatibility aliases only. When an Outlook connector is available, create the time block there and save its returned IDs and link on the task. Do not create sermon-writing, sermon-preparation, preaching, or automatic sermon-completion tasks; use Sermon Workspace for that lifecycle.",
    handler: createTask
  }),
  defineOperation({
    name: "updateTask",
  ...
  defineOperation({
    ...
    required: ["title"],
    optional: ["projectId", "maintenance", "routineId", "lifeArea", "status", "recurrence", "recurrenceNotes", "preferredTime", "assignedTo", "requestedBy", "notes", "visibility"],
    exampleArguments: { title: "Check mail and packages", lifeArea: "home", recurrence: "daily", preferredTime: "after work" },
    argumentGuidance: "Maintenance fields require a project under proj-fbc-maintenance: maintenance:{building,area,thirdParty,workflowStatus,cost:{estimate:null|number,actual:null|number,currency:USD,note},targetDateText,sourceStatus,sourcePriority,sourceReferences:[]}. Updating maintenance replaces that object; preserve existing values. workflowStatus: Not Started, Planning, Assessment Needed, Quote Needed, Quote / Schedule, Waiting, In Progress, Completed, Deferred. Completed work requires task status done after explicit approval. ",
