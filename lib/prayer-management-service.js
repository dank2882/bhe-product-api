...
function isDue(schedule, iso, lastPrayedAt = "") {
  ...
  return !lastPrayedDate || lastPrayedDate < occurrence;
}
...
async function getTodaysPrayers(input = {}, deps = {}) {
  ...
  const prayers = result.prayers.filter((p) => isDue(p.schedule, at, p.lastPrayedAt));
