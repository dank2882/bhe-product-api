...
function owner(deps = {}) {
  ...
  const subjects = [...new Set([subject, ...(Array.isArray(access.subjects) ? access.subjects : []), ...delegatedOwnerSubjects(subject, deps.privateDelegationEnv)].map(normalizeString).filter(Boolean))];
...
function normalizeSchedule(value = {}) {
  value = value ?? {};
  const requestedKind = normalizeString(value.kind || "daily").toLowerCase();
  // Preserve old clients while retiring the always-due unscheduled behavior.
  const kind = requestedKind === "unscheduled" ? "daily" : requestedKind;
  if (!["daily", "weekly", "monthly", "interval", "date"].includes(kind)) fail("Unsupported prayer schedule kind", 400, "invalid_prayer_schedule");
...
async function decryptRecord(record, recordType, deps) {
  ...
  return { ...content, ...metadata, ...(recordType === "prayer" ? { schedule: normalizeSchedule(metadata.schedule) } : {}) };
}
