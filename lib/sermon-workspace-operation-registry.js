...
const preachingProfile = require("./sermon-preaching-profile-service");
...
const SERMON_WORKSPACE_OPERATIONS = Object.freeze([
  defineOperation({
    name: "listSermonSeries",
    mode: "query",
    summary: "List first-class Series Hubs with their durable burden, approach, date window, and planning status.",
    optional: ["query", "status", "limit"],
    exampleArguments: { status: "active", limit: 20 },
    handler: sermonWorkspace.listSermonSeries
  }),
  defineOperation({
    name: "getSermonSeries",
    mode: "query",
    summary: "Retrieve one complete Series Hub, its provisional message map, linked sermon hubs, exact development turns, and reconciliation gaps.",
    optional: ["seriesId", "seriesSlug", "seriesTitle", "folderId"],
    exampleArguments: { seriesId: "series-stewarding-your-life" },
    argumentGuidance: "Send one stable series identity. Use the returned version for any update or development-turn append.",
    handler: sermonWorkspace.getSermonSeries
  }),
  defineOperation({
    name: "listSermons",
    mode: "query",
  ...
  defineOperation({
    ...
    mode: "query",
    summary: "Retrieve the durable, versioned preaching profile used by sermon-development and manuscript workflows.",
    optional: ["profileId"],
    exampleArguments: {},
    handler: sermonWorkspace.getPreachingProfile
  }),
  defineOperation({
    name: "getPreachingProfileBaselineReadiness",
    mode: "query",
    summary: "Select a deduplicated, recent-first representative transcript corpus and report whether a preaching-profile baseline or cadence review is ready.",
    optional: ["profileId", "limit", "asOfDate"],
    exampleArguments: { limit: 12 },
    argumentGuidance: "Recent sermons define current voice; older sermons are comparison evidence. Lower-fidelity translator, partial, continuation, and raw sources are excluded when stronger transcript evidence exists.",
    handler: preachingProfile.getPreachingProfileBaselineReadiness
  }),
  defineOperation({
    name: "proposePreachingProfileBaseline",
    mode: "query",
    summary: "Build a read-only evidence-grounded preaching fingerprint, context guidance, and growth baseline from representative transcripts.",
    optional: ["profileId", "limit", "asOfDate"],
    exampleArguments: { limit: 12 },
    argumentGuidance: "Proposal only. Present the complete proposed profile and wait for Dan's explicit approval before applyPreachingProfileBaseline.",
    handler: preachingProfile.proposePreachingProfileBaseline
  }),
  defineOperation({
  ...
  defineOperation({
    name: "createSermonSeries",
    mode: "command",
    summary: "Create or adopt a first-class Series Hub inside the existing Sermon Workspace.",
    ...
    argumentGuidance: "Preserve Dan's complete initiating wording in sourceIdea. This creates the series-level plan; individual passages, big ideas, manuscripts, and occasions remain on sermon hubs.",
    handler: sermonWorkspace.createSermonSeries
  }),
  defineOperation({
    name: "createSermon",
    mode: "command",
  ...
  defineOperation({
    name: "updateSermonSeries",
    mode: "command",
    summary: "Version-update canonical Series Hub fields or replace its provisional message map without rewriting linked sermons.",
    required: ["seriesId", "expectedVersion", "changes"],
    exampleArguments: {
      seriesId: "series-stewarding-your-life",
      expectedVersion: 1,
      changes: { pastoralBurden: "Everything entrusted by God should be offered back to Him faithfully." }
    },
    argumentGuidance: "Read getSermonSeries first and pass its exact version. seriesId, seriesSlug, and an existing exact sourceIdea are immutable. Changing the series plan never silently rewrites child sermon hubs.",
    handler: sermonWorkspace.updateSermonSeries
  }),
  defineOperation({
    name: "appendSermonSeriesDevelopmentTurn",
    mode: "command",
    summary: "Append one exact Dan or assistant turn to a Series Hub before shaping the durable series plan.",
    ...
    argumentGuidance: "Capture Dan's complete words before retrieval or substantive shaping. Read back the returned version before appending another turn.",
    handler: sermonWorkspace.appendSermonSeriesDevelopmentTurn
  }),
  defineOperation({
  ...
  defineOperation({
    ...
    optional: ["profileId"],
    exampleArguments: { expectedVersion: 1, changes: { draftingGuidance: "Preserve a warm and direct pastoral voice." } },
    argumentGuidance: "Read getPreachingProfile first and pass its exact current version. Profile baselines must use the proposal/apply workflow instead of this direct update.",
    handler: sermonWorkspace.updatePreachingProfile
  }),
  defineOperation({
    name: "applyPreachingProfileBaseline",
    mode: "command",
    summary: "Apply an unchanged, Dan-approved preaching-profile baseline proposal after verifying profile version and transcript fingerprints.",
    ...
    exampleArguments: {
      ...
      sourceFingerprint: "sha256-from-proposal",
      corpus: [],
      profile: { summary: "Reviewed profile" },
      confirmed: true
    },
    argumentGuidance: "Use only after Dan approves the exact proposal. Send the proposal values unchanged; stale profile or transcript evidence is rejected.",
    handler: preachingProfile.applyPreachingProfileBaseline
  }),
  defineOperation({
