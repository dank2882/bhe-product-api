...
const STAFF_TEAM_IDS = Object.freeze([
  "bhe",
  "administration",
  "school",
  "finance",
...
const STAFF_TEAM_ROLES = Object.freeze(["manager", "member"]);

const STAFF_AUTHORIZATION_ROLE_BUNDLES = Object.freeze({
  "FBC Staff Tools Administrator": {
    permissions: STAFF_AUTHORIZATION_SCOPES.filter((scope) => !scope.startsWith("prayer.") && !scope.startsWith("shipping.")),
    taskRole: "admin"
  },
  ...
  "FBC Shipping Owner": {
    permissions: ["shipping.read", "shipping.write", "shipping.approve", "shipping.documents.restricted"]
  },
  ...
  "BHE Pastoral Care Worker": {
    permissions: ["care.read.main", "care.search", "care.records.read"]
  },
  "BHE Spanish Pastor": {
...
function normalizeTeamMemberships(value = []) {
  if (!Array.isArray(value)) {
    throw createAuthorizationError(
      "Staff team memberships must be an array",
      400,
      "invalid_staff_team_memberships"
    );
  }
  const seen = new Set();
  return value.map((membership, index) => {
    if (!membership || typeof membership !== "object" || Array.isArray(membership)) {
      throw createAuthorizationError("Each team membership must be an object", 400, "invalid_staff_team_membership", { index });
    }
    const teamId = normalizeString(membership.teamId).toLowerCase();
    const role = normalizeString(membership.role).toLowerCase() || "member";
    const status = normalizeString(membership.status).toLowerCase() || "active";
    if (!STAFF_TEAM_IDS.includes(teamId) || !STAFF_TEAM_ROLES.includes(role) || !["active", "disabled"].includes(status)) {
      throw createAuthorizationError("Invalid staff team membership", 400, "invalid_staff_team_membership", {
        index, teamId, role, status, allowedTeamIds: STAFF_TEAM_IDS, allowedRoles: STAFF_TEAM_ROLES
      });
    }
    if (seen.has(teamId)) {
      throw createAuthorizationError("A staff profile may contain only one membership per team", 400, "duplicate_staff_team_membership", { teamId });
    }
    seen.add(teamId);
    return { teamId, role, status };
...
function buildStaffAuthorizationSummary(profile = {}, fallbackId = "") {
  ...
  return {
    ...
    taskRole: STAFF_TASK_ROLES.includes(profile.taskRole)
      ? profile.taskRole
      : "viewer",
    teamMemberships: normalizeTeamMemberships(
      Array.isArray(profile.teamMemberships) ? profile.teamMemberships : []
    ),
...
async function resolveStaffAuthorization(input = {}, deps = {}) {
  ...
  return {
    ...
    effectiveTeamMemberships: isActive
      ? summary.teamMemberships.filter(({ status }) => status === "active")
      : [],
    effectiveTeamIds: isActive
      ? summary.teamMemberships.filter(({ status }) => status === "active").map(({ teamId }) => teamId)
      : []
  };
}
...
async function saveStaffAuthorizationProfile(input = {}, deps = {}) {
  ...
  const next = {
    ...
    teamMemberships: Object.prototype.hasOwnProperty.call(changes, "teamMemberships")
      ? normalizeTeamMemberships(changes.teamMemberships)
      : normalizeTeamMemberships(existing?.teamMemberships || []),
