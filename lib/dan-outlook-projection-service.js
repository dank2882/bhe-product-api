...
async function recordOutlookContactPublish(input = {}, deps = {}) {
  ...
  const existingDoc = await collection.doc(built.person.personId).get();
  ...
  await collection.doc(built.person.personId).set(projection);
  return { projection };
}
...
async function observeOutlookContact(input = {}, deps = {}) {
  requireDanPrivateAccess(deps);
  const collection = getCollection(deps, "outlookProjectionsCollection");
  const projection = await getRequiredRecord(collection, input.personId, "outlookProjection");
  ...
  if (input.deleted === true || !outlookSnapshot.displayName) {
    ...
    await collection.doc(projection.personId).set(next);
...
async function replaceContactMethods(personId, type, values, deps) {
  const collection = getCollection(deps, "relationshipContactMethodsCollection");
  const existing = (await loadCollection(collection)).filter((item) => item.personId === personId && item.type === type && (item.status || "active") === "active");
  const now = getNowIso(deps);
  for (const item of existing) {
    await collection.doc(item.contactMethodId).set({ ...item, status: "archived", version: Number(item.version || 0) + 1, updatedAt: now });
  }
  const actor = getDanActorFields(deps);
  for (const [index, value] of values.map(normalizeString).filter(Boolean).entries()) {
    ...
    await collection.doc(contactMethodId).create({
      contactMethodId,
      personId,
      organizationId: "",
      owner: "dan",
...
async function resolveOutlookMerge(input = {}, deps = {}) {
  ...
  const projection = await getRequiredRecord(projections, input.personId, "outlookProjection");
  assertExpectedVersion(projection, input.expectedVersion, "outlookProjection");
  ...
  const person = await getRequiredRecord(people, projection.personId, "person");
  ...
  if (replaceEmailsFromOutlook) {
    await replaceContactMethods(
      projection.personId,
      "email",
      (chosenValue("emailAddresses") || []).map((item) => item.address),
      deps
    );
  }
  if (replacePhonesFromOutlook) {
    await replaceContactMethods(
      projection.personId,
      "phone",
      [chosenValue("mobilePhone"), ...(chosenValue("businessPhones") || [])],
      deps
    );
  }
  if (stableStringify(nextPerson) !== stableStringify(person)) {
    ...
    nextPerson.updatedAt = getNowIso(deps);
  ...
  await projections.doc(projection.personId).set(nextProjection);
  return {
...
async function recordOutlookPhotoPublish(input = {}, deps = {}) {
  ...
  await projections.doc(projection.personId).set(next);
