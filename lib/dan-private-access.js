...
function requireDanPrivateAccess(deps = {}, { allowAutomation = false } = {}) {
  ...
  const allowedSubjects = [...access.subjects, ...delegatedOwnerSubjects(access.subject, deps.privateDelegationEnv)];
  const matchedSubject = allowedSubjects.find((subject) => ownerSubjects.includes(subject));
  if (!matchedSubject) {
    throw createDanAccessError(
