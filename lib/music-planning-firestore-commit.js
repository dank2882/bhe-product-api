...
const SAFE_PARTIAL_CONFLICT_REASONS = new Set([
  "existing_completed_confirmed_or_changed_after_plan",
  "existing_record_not_spreadsheet_owned_planned"
]);
...
function getPlanConflicts(plan = {}) {
  if (Array.isArray(plan.conflicts)) {
    return plan.conflicts;
  }

  return [
    ...(Array.isArray(plan.services?.conflict) ? plan.services.conflict : []),
    ...(Array.isArray(plan.serviceSongEvents?.conflict) ? plan.serviceSongEvents.conflict : [])
  ];
}

function getUnsafePartialConflicts(plan = {}) {
  return getPlanConflicts(plan).filter((conflict) => {
    const reason = normalizeString(conflict.reason);
    return !SAFE_PARTIAL_CONFLICT_REASONS.has(reason);
  });
}
...
function validateMusicPlanningCommitPlan(plan = {}, options = {}) {
  ...
  const unsafePartialConflicts = getUnsafePartialConflicts(plan);
  ...
  if (plan.eligibleForCommit !== true && (!allowPartialConflicts || unsafePartialConflicts.length > 0)) {
    errors.push("Plan is not eligible for commit.");
  }

  const planConflicts = getPlanConflicts(plan);
  const conflictCount = planConflicts.length > 0
    ? planConflicts.length
    : Number(plan.summary && plan.summary.conflicts && plan.summary.conflicts.total) || 0;
  if (conflictCount > 0 && !allowPartialConflicts) {
    errors.push("Plan contains blocking conflicts.");
  }

  if (allowPartialConflicts && unsafePartialConflicts.length > 0) {
    errors.push(
      `Plan contains ${unsafePartialConflicts.length} conflicts that are not safe for partial commit.`
    );
  }
