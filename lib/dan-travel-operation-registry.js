...
async function recordAuditEvent(operation, args, result, deps) {
  ...
  await collection.doc(auditEventId).create({
    auditEventId,
    owner: "dan",
    visibility: "private",
    operation,
...
async function runDanTravelOperation(input = {}, deps = {}) {
  ...
  if (mode === "command") await recordAuditEvent(operationName, args, result, deps);
