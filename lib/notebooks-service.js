...
function chunks(note) {
  const result = [];
  const sections = [["title", note.title], ["note_text", note.text], ["tags", note.tags.join(", ")],
    ["links", note.links.join("\n")], ["assistant_summary", note.assistantSummary]];
  for (const [sourceKind, source] of sections) {
    for (let offset = 0; offset < source.length; offset += CHUNK_SIZE - CHUNK_OVERLAP) {
      ...
      result.push({ chunkId, noteId: note.noteId, notebookId: note.notebookId, ownerSubject: note.ownerSubject,
        revisionId: note.revisionId, version: note.version, sourceKind,
        text: passage, tokens: tokens(passage), textHash: hash(passage) });
...
async function searchNotes(input, deps) {
  ...
  const lowerDate = afterDate ? new Date(afterDate).toISOString() : "";
  const upperDate = beforeDate ? new Date(beforeDate.length === 10 ? `${beforeDate}T23:59:59.999Z` : beforeDate).toISOString() : "";
  if (lowerDate && upperDate && lowerDate > upperDate) fail("updatedFrom must precede updatedTo");
  ...
  for (const chunk of candidates.values()) {
    ...
    if (lowerDate && note.updatedAt < lowerDate || upperDate && note.updatedAt > upperDate) continue;
    ...
    if (!matches.has(note.noteId) || matches.get(note.noteId).score < score) matches.set(note.noteId, {
      ...
      revisionId: note.revisionId, version: note.version, excerpt: chunk.text, excerptSource: chunk.sourceKind,
