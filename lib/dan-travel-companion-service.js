...
async function buildDestinationRefresher(input = {}, deps = {}) {
  ...
  const locationMatchedInteractions = interactions.filter((item) => (
    Array.isArray(item.locationKeys)
    && item.locationKeys.length > 0
    && locationMatches(item.locationKeys, destination)
  ));
  const interactionPersonIds = new Set(locationMatchedInteractions.flatMap((item) => item.personIds || []));
  const interactionOrganizationIds = new Set(locationMatchedInteractions.flatMap((item) => item.organizationIds || []));
  const matchingOrganizations = organizations.filter((item) => (
    locationMatches(item.locationKeys, destination)
    || interactionOrganizationIds.has(item.organizationId)
  ));
  ...
  const matchingPeople = people.filter((person) => (
    locationMatches(person.locationKeys, destination)
    || affiliatedPersonIds.has(person.personId)
    || interactionPersonIds.has(person.personId)
  ));
