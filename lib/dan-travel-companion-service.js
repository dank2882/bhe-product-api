...
async function updateTrip(input = {}, deps = {}) {
  ...
  const existing = await getRequiredRecord(collection, input.tripId, "trip");
  ...
  const next = { ...existing };
  const existingDestinationIds = new Set((existing.destinations || []).map((item) => item.destinationId));
  ...
  if (Object.prototype.hasOwnProperty.call(changes, "destinations")) next.destinations = normalizeDestinations(changes.destinations);
  if (Object.prototype.hasOwnProperty.call(changes, "startDate")) next.startDate = normalizeDate(changes.startDate, "trip_start_date");
  if (Object.prototype.hasOwnProperty.call(changes, "endDate")) next.endDate = normalizeDate(changes.endDate, "trip_end_date");
  if (Object.prototype.hasOwnProperty.call(changes, "travelers")) next.travelers = normalizeStrings(changes.travelers, 25);
  if (!TRIP_STATUSES.includes(next.status)) throw createRelationshipError("Invalid trip status", 400, "invalid_trip_status");
  if (next.startDate && next.endDate && next.endDate < next.startDate) throw createRelationshipError("Invalid trip date range", 400, "trip_date_range_invalid");
  next.searchText = normalizeKey([next.name, next.purpose, next.destinations.map((item) => item.name), next.notes].flat().join(" "));
  next.version = Number(existing.version || 0) + 1;
  next.updatedAt = getNowIso(deps);
  await collection.doc(existing.tripId).set(next);
...
async function recordItineraryCalendarExport(input = {}, deps = {}) {
  ...
  const item = await getRequiredRecord(collection, input.itineraryItemId, "itineraryItem");
  ...
  await collection.doc(item.itineraryItemId).set(next);
...
async function updatePackingItem(input = {}, deps = {}) {
  ...
  const list = await getRequiredRecord(collection, input.packingListId, "packingList");
  ...
  await collection.doc(list.packingListId).set(next);
  return { packingListId: list.packingListId, packingItem: item, version: next.version };
}

function locationMatches(recordKeys, destinationKeys) {
  const record = (recordKeys || []).map(normalizeKey).filter(Boolean);
  const destination = (destinationKeys || []).map(normalizeKey).filter(Boolean);
  return record.some((left) => destination.some((right) => left.includes(right) || right.includes(left)));
}
...
async function buildDestinationRefresher(input = {}, deps = {}) {
  ...
  const matchingOrganizations = organizations.filter((item) => locationMatches(item.locationKeys, destination.locationKeys));
  ...
  const matchingPeople = people.filter((person) => locationMatches(person.locationKeys, destination.locationKeys) || affiliatedPersonIds.has(person.personId));
  const personIds = new Set(matchingPeople.map((person) => person.personId));
  const relevantInteractions = interactions
    .filter((item) => item.tripId === trip.tripId || locationMatches(item.locationKeys, destination.locationKeys) || (item.personIds || []).some((id) => personIds.has(id)))
