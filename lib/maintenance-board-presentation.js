...
function renderTable(rows, range, total, hasMore, routine) {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    cells.Images = cells.Images.length ? `${cells.Images.length} photo(s)` : "None attached";
    if (row.notesTruncated) cells.Notes += " [full notes available]";
    output.push(`| ${COLUMNS.map(column => escapeCell(cells[column])).join(" | ")} |`);
