...
function renderTable(rows, range, total, hasMore, routine) {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    cells.Task = `${row.number}. ${cells.Task} · ${row.reference}`;
  ...
  if (photoRows.length) output.push("Ask to open a task’s photo viewer for thumbnails and enlargement. Open photo and Download links expire after 15 minutes.", "");
  output.push("Use the numbers in this table, for example “delete 1, 3, 5” (archive) or “mark 2 complete.” Numbers apply to this displayed list and can change after refresh or filtering. Names and permanent references also work. Saved changes appear when the table is refreshed.");
  return output.join("\n");
}
...
function presentBoard(allRows, input = {}, routine = false) {
  ...
  const page = rows.slice(start, start + limit).map((row, index) => ({ ...row, number: start + index + 1 })), hasMore = start + page.length < rows.length;
  ...
  return {
    ...
    selection: { snapshot, recordType: routine ? "routine" : "task", items: page.map(row => ({ number: row.number, [routine ? "routineId" : "taskId"]: row[routine ? "routineId" : "taskId"], version: row.version, reference: row.reference })) },
    ...
    guidance: "Display markdown directly as an in-chat working table, not a download or code block. Preserve all ten columns including Images, photo counts, and open/download links; do not omit them when summarizing. Preserve displayed item numbers, grouping, references and page counts. Default to one readable page; fetch more when requested, and follow all pages only for an explicit complete list. Never call a page the complete list. Numbered commands refer to the latest working table actually displayed to the user, not a later hidden query. Resolve ALL requested numbers to IDs from that table’s selection.items BEFORE any write, and keep those IDs fixed throughout the batch; never refresh/reassign numbers between writes. Use numbers only from the same selection.snapshot and recordType; if the original displayed mapping is unavailable, a number is absent, or task/routine lists are ambiguous, show a fresh table and ask which items before writing. Never guess from current sorted positions. Query by permanent reference to recover an exact record. For Maintenance, “delete 1, 3, 5” means recoverable archive: updateTask status dropped or updateRoutine status archived, never permanentlyDelete. Explain archive in the receipt, report each success/failure, and refresh once after the batch. Before an authorized update, retrieve current details/version, use existing versioned Task Management commands with an idempotency key, then read back each affected record; refresh the displayed table after all selected updates finish. Assignment proposals are not accepted assignments. Compact Notes are excerpts; retrieve getTask or listMaintenanceRoutines with detailLevel full and reference for source history. Photo counts reflect actual attachments, not source placeholders. Signed photo URLs expire; refresh the query for fresh links. For requested photo previews call tasks_open_maintenance_photos with the exact M-reference and includeArchived when requested. The embedded viewer supplies thumbnails, enlargement and refresh. Do not construct Markdown image previews: they failed ChatGPT acceptance. Text-only clients can use listAttachments and getAttachmentDownload and offer open/download links. Recurring completion uses recordMaintenanceRoutineCompletion, not task completion. No automatic messages or spreadsheet exports. Markdown cells are not directly editable; accept changes through chat."
  };
}
