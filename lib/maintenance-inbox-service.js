...
async function reviewMaintenanceMessage(input, deps) {
  ...
  if (!task) {
    ...
    try {
      task = (await createTask({ taskId: target, projectId: ROOT_ID, lifeArea: "church", title: f.title, assignedTo: "", assignedToSub: "", status: "next", priority: "medium", requestedBy: approved.sender,
