...
const { normalizeMaintenance, validateMaintenanceParent, isMaintenanceProject } = require("./maintenance-fields");
const { readCompleteQuery } = require("./complete-query");
const { withProjectContext, ancestry, branchIds, saveHierarchyProject, hierarchyError } = require("./project-hierarchy");
const {
  ...
  isBranchRecord,
  getBranchRole,
  getCollaborativeBranch,
  isCollaborativeBranchOwner,
  isTaskAdmin,
  normalizeVisibility
} = require("./task-management-access");
const { buildStaffAuthorizationSummary } = require("./staff-authorization-service");
...
const STAFF_TEAMS = Object.freeze([
  { teamId: "bhe", displayName: "BHE", version: 1, status: "active" },
  { teamId: "administration", displayName: "Administration", version: 1, status: "active" },
  { teamId: "school", displayName: "School", version: 1, status: "active" },
  { teamId: "finance", displayName: "Finance", version: 1, status: "active" },
  { teamId: "maintenance", displayName: "Maintenance", version: 1, status: "active" },
  { teamId: "it", displayName: "I.T.", version: 1, status: "active" },
  { teamId: "operations", displayName: "Operations", version: 1, status: "active" },
  { teamId: "web", displayName: "Web", version: 1, status: "active" }
]);
...
const STAFF_LEADERSHIP_AREAS = ["bhe", "missions"];
...
function normalizeLeadershipAreas(value = []) {
  if (!Array.isArray(value)) {
    throw createProjectTaskError(
      "Staff leadership areas must be an array",
      400,
      { value, allowedValues: STAFF_LEADERSHIP_AREAS },
      "invalid_staff_leadership_areas"
    );
  }
  const normalized = [...new Set(value.map((item) => normalizeString(item).toLowerCase()).filter(Boolean))];
  const unsupported = normalized.filter((item) => !STAFF_LEADERSHIP_AREAS.includes(item));
  if (unsupported.length) {
    throw createProjectTaskError(
      "Invalid staff leadership area",
      400,
      { unsupported, allowedValues: STAFF_LEADERSHIP_AREAS },
      "invalid_staff_leadership_areas"
    );
  }
...
function normalizeTeamId(value) {
  const teamId = normalizeString(value).toLowerCase();
  if (!teamId) return "";
  if (!STAFF_TEAMS.some((team) => team.teamId === teamId)) {
    throw createProjectTaskError("Invalid staff team", 400, {
      fieldName: "teamId", value: teamId, allowedValues: STAFF_TEAMS.map(({ teamId: id }) => id)
    }, "invalid_team_id");
  }
  return teamId;
}
...
async function listTeams() {
  return { teams: clone(STAFF_TEAMS) };
}
...
function buildProjectSummary(project = {}, fallbackId = "") {
  return {
    ...
    collaborationPolicy: project.collaborationPolicy || "",
    branchMembers: Array.isArray(project.branchMembers) ? clone(project.branchMembers) : [],
    name: project.name || "",
    ...
    archivedBySub: project.archivedBySub || "",
    archivedByName: project.archivedByName || "",
    lastArchivedAt: project.lastArchivedAt || project.archivedAt || "",
...
function buildTaskSummary(task = {}, fallbackId = "") {
  ...
  return {
    ...
    title: task.title || "",
    maintenance: task.maintenance || null,
    ...
    archivedBySub: task.archivedBySub || "",
    archivedByName: task.archivedByName || "",
    lastArchivedAt: task.lastArchivedAt || task.droppedAt || "",
...
function buildRoutineSummary(routine = {}, fallbackId = "") {
  return {
    ...
    maintenance: routine.maintenance || null,
    lastCompletedAt: routine.lastCompletedAt || "",
    lastCompletedBySub: routine.lastCompletedBySub || "",
    archivedBySub: routine.archivedBySub || "",
    archivedByName: routine.archivedByName || "",
    lastArchivedAt: routine.lastArchivedAt || "",
    title: routine.title || "",
...
async function loadCollection(collectionRef) {
  return (await readCompleteQuery(collectionRef)).map((doc) => ({
    id: doc.id, data: doc.data() || {}, snapshot: doc
  }));
}

async function saveObservedRecord(docRef, nextRecord, snapshot) {
  if (snapshot.updateTime && typeof docRef.update === "function") {
    try {
      await docRef.update(nextRecord, { lastUpdateTime: snapshot.updateTime });
    } catch (error) {
      if ([9, 10].includes(Number(error.code))) {
        throw createProjectTaskError("The record changed during this update; reload it and retry", 409, {}, "task_version_conflict");
      }
      throw error;
    }
    return;
  }
  // In-memory test adapters have no Firestore timestamps. Production must never
  // fall back to an unconditional write when its snapshot is missing a version.
  if (docRef.firestore) throw createProjectTaskError("A versioned database snapshot is required", 500);
  await docRef.set(nextRecord);
}

function resultPage(records, input, idField, limit) {
  let start = 0;
  if (input.cursor) {
    let cursor;
    try { cursor = JSON.parse(Buffer.from(input.cursor, "base64url").toString()); } catch { /* invalid below */ }
    const query = { ...input }; delete query.cursor; delete query.limit;
    const key = createHash("sha256").update(JSON.stringify(Object.keys(query).sort().map(k => [k, query[k]]))).digest("hex");
    if (!cursor || cursor.key !== key || !records.some(r => r[idField] === cursor.id)) {
      throw createProjectTaskError("The result cursor is invalid or stale; restart the query", 409, {}, "query_cursor_stale");
    }
    start = records.findIndex(r => r[idField] === cursor.id) + 1;
  }
  const page = records.slice(start, start + limit);
  const hasMore = start + page.length < records.length;
  const query = { ...input }; delete query.cursor; delete query.limit;
  const key = createHash("sha256").update(JSON.stringify(Object.keys(query).sort().map(k => [k, query[k]]))).digest("hex");
  return { page, hasMore, nextCursor: hasMore ? Buffer.from(JSON.stringify({ key, id: page.at(-1)[idField] })).toString("base64url") : null };
}
...
function buildStaffProfileSummary(profile = {}, fallbackId = "") {
  return {
    ...
    leadershipAreas: normalizeLeadershipAreas(profile.leadershipAreas || []),
...
async function saveProjectWithDepartmentCascade({
  ...
  if (!departmentChanged) {
    await saveObservedRecord(docRef, nextProject, snapshot);
    return 0;
  }
  ...
  const taskRecords = (await loadCollection(tasksCollection, 2000))
    ...
    .filter(({ data }) =>
      normalizeString(data.department).toLowerCase() !== nextProject.department
      || normalizeString(data.teamId).toLowerCase() !== nextProject.teamId
    )
    .map(({ id, data, snapshot }) => {
      const task = {
        ...clone(data),
        taskId: id,
      ...
      return { id, task, snapshot };
    });

  const firestore = docRef.firestore || tasksCollection.firestore;
  if (firestore && taskRecords.length > 450) {
    throw createProjectTaskError("This project needs a dedicated migration for its classification change", 422, { taskCount: taskRecords.length }, "project_cascade_too_large");
  }
  if (firestore && typeof firestore.batch === "function") {
    const batch = firestore.batch();
    batch.update(docRef, nextProject, { lastUpdateTime: snapshot.updateTime });
    for (const { id, task, snapshot: taskSnapshot } of taskRecords) {
      batch.update(tasksCollection.doc(id), task, { lastUpdateTime: taskSnapshot.updateTime });
    }
    try { await batch.commit(); } catch (error) {
      if ([9, 10].includes(Number(error.code))) throw createProjectTaskError("Project or linked task changed; reload and retry", 409, {}, "task_version_conflict");
      throw error;
    }
  } else {
...
function normalizeParentProjectId(value) {
  if (value === undefined) return "";
  if (typeof value !== "string") throw hierarchyError("parentProjectId must be a project ID or an empty string");
  return value.trim() ? validateDocId(value, "parentProjectId") : "";
}

function normalizeBranchMembers(input) {
  if (input === undefined) return [];
  if (!Array.isArray(input) || input.length > 100) throw hierarchyError("branchMembers must contain at most 100 people");
  const seen = new Set();
  return input.map(member => {
    const subject = normalizeString(member?.subject);
    if (!subject || subject.length > 500 || !["viewer", "editor"].includes(member.role) || seen.has(subject)) throw hierarchyError("Each branch member needs a unique authenticated subject and viewer/editor role");
    seen.add(subject); return { subject, role: member.role };
  });
}

function assertCanManageBranch(project, deps) {
  if (getCollaborativeBranch(project, deps)) {
    if (isCollaborativeBranchOwner(project, deps)) return;
    throw createProjectTaskError("Only the team area owner may change sharing or move this branch", 403, {}, "project_sharing_denied");
  }
  let current = project;
  const seen = new Set();
  while (current && !seen.has(current.projectId)) {
    if (isTaskAdmin(deps) || isRecordOwner(current, deps)) return;
    seen.add(current.projectId);
    if (current.visibility !== "branch") break;
    current = deps.projectGraph?.get(current.parentProjectId);
  }
  throw createProjectTaskError("Only the project owner or administrator may change branch sharing or move a restricted branch", 403, {}, "project_sharing_denied");
}

function assertBranchStructure(next, graph) {
  const candidate = new Map(graph); candidate.set(next.projectId, next);
  for (const id of branchIds(next.projectId, candidate)) {
    const project = candidate.get(id), parent = candidate.get(project.parentProjectId);
    if (parent?.visibility === "branch" && project.visibility !== "branch") throw hierarchyError("Projects under a restricted branch must also use branch visibility. Convert existing child projects explicitly before linking them.", "project_branch_visibility_conflict", 409);
  }
  if (next.visibility !== "branch" && (next.branchMembers || []).length) throw hierarchyError("Branch members require branch visibility");
}

function assertHierarchyChangeAllowed(next, existing, deps) {
  if (existing) {
    assertCanUpdateTaskRecord(existing, {}, deps, "project");
    if (existing.visibility === "branch" || next.visibility === "branch") assertCanManageBranch(existing, deps);
  }
  assertBranchStructure(next, deps.projectGraph);
  if (next.parentProjectId) {
    const parent = deps.projectGraph.get(next.parentProjectId);
    if (!parent) throw hierarchyError("Parent project was not found", "project_not_found", 404);
    assertCanUpdateTaskRecord(parent, {}, deps, "project");
  }
}

async function buildProjectHierarchy(projectId, input, deps) {
  deps = await withProjectContext(deps);
  const selected = branchIds(projectId, deps.projectGraph);
  const visible = new Map([...deps.projectGraph].filter(([id, project]) => selected.has(id) && canReadTaskRecord(project, deps)));
  if (visible.size > 250) throw hierarchyError("This overview exceeds 250 visible projects; choose a smaller branch", "project_overview_too_large", 422);
  const today = normalizeOptionalDate(input.today, "today") || getTodayDate(deps);
  const tasks = (await loadCollection(getTaskCollection(deps))).map(({ id, data }) => buildTaskSummary(data, id))
    .filter(task => visible.has(task.projectId) && canReadTaskRecord(task, deps));
  const counts = records => ({
    total: records.length,
    open: records.filter(task => !["done", "dropped"].includes(task.status)).length,
    next: records.filter(task => task.status === "next").length,
    waiting: records.filter(task => task.status === "waiting").length,
    done: records.filter(task => task.status === "done").length,
    dropped: records.filter(task => task.status === "dropped").length,
    overdue: records.filter(task => !["done", "dropped"].includes(task.status) && task.dueDate && task.dueDate < today).length
  });
  const projects = [...visible.values()].map(project => {
    const descendants = branchIds(project.projectId, deps.projectGraph);
    const ownTasks = tasks.filter(task => task.projectId === project.projectId);
    const nestedTasks = tasks.filter(task => descendants.has(task.projectId));
    const path = ancestry(project.projectId, deps.projectGraph).reverse().filter(node => canReadTaskRecord(node, deps));
    return {
      ...buildBriefProject(project),
      depth: ancestry(project.projectId, deps.projectGraph).length - 1,
      path: path.map(node => ({ projectId: node.projectId, name: node.name })),
      directTaskCounts: counts(ownTasks),
      branchTaskCounts: counts(nestedTasks),
      childProjectCount: [...visible.values()].filter(node => node.parentProjectId === project.projectId).length,
      nextTasks: ownTasks.filter(task => task.status === "next").sort((a,b) => (a.dueDate || "9999").localeCompare(b.dueDate || "9999") || a.taskId.localeCompare(b.taskId)).slice(0, 5).map(buildBriefTask),
      moreNextTasks: ownTasks.filter(task => task.status === "next").length > 5
    };
  }).sort((a,b) => a.path.map(p => p.name).join(" / ").localeCompare(b.path.map(p => p.name).join(" / ")) || a.projectId.localeCompare(b.projectId));
  return {
    rootProjectId: projectId, today, coverage: "accessible_records_only", projectCount: projects.length,
    taskCounts: counts(tasks),
    projectCounts: Object.fromEntries(PROJECT_STATUSES.map(status => [status, [...visible.values()].filter(project => (project.status || "active") === status).length])),
    projects,
    guidance: "Each task is counted once in the selected branch total. Branch totals overlap; do not add parent and child totals. Project status is set explicitly and does not close child projects or tasks. Use listTasks with projectId and includeDescendants for the full task list."
  };
}
...
async function createProject(input = {}, deps = {}) {
  ...
  const parent = input.parentProjectId ? deps.projectGraph?.get(input.parentProjectId) : null;
  input = { ...input };
  if (parent?.visibility === "branch" && !input.visibility) input.visibility = "branch";
  const branchMembers = normalizeBranchMembers(input.branchMembers);
  const collaborationPolicy = normalizeString(input.collaborationPolicy);
  if (collaborationPolicy && (!["editor_archive_creator_delete", "editor_archive_owner_delete"].includes(collaborationPolicy) || input.visibility !== "branch")) throw hierarchyError("The collaboration policy requires branch visibility");
  if ((collaborationPolicy || branchMembers.length) && parent && getCollaborativeBranch(parent, deps)) assertCanManageBranch(parent, deps);
  if (branchMembers.length && input.visibility !== "branch") throw hierarchyError("Choose branch visibility when sharing with specific people");
  ...
  const project = {
    projectId,
    parentProjectId: normalizeParentProjectId(input.parentProjectId),
    branchMembers,
    collaborationPolicy,
    name,
  ...
  if (project.status === "archived") {
    ...
    project.lastArchivedAt = nowIso;
    project.archivedBySub = actor.actorSub;
    project.archivedByName = actor.actorName;
  }

  project.searchText = buildProjectSearchText(project);
  if (project.parentProjectId) {
    await saveHierarchyProject({ collection: projectsCollection, next: project, create: true,
      authorize: graph => assertHierarchyChangeAllowed(project, null, { ...deps, projectGraph: graph }) });
  } else await docRef.create(project);
...
async function listProjects(input = {}, deps = {}, allResults = false) {
  const projectsCollection = getProjectCollection(deps);
  ...
  deps = await withProjectContext(deps);
  let selectedIds;
  if (input.ancestorProjectId) {
    const anchor = validateDocId(input.ancestorProjectId, "ancestorProjectId");
    await assertProjectReferenceAccessible(anchor, deps);
    selectedIds = branchIds(anchor, deps.projectGraph);
  }
  if (input.parentProjectId) await assertProjectReferenceAccessible(validateDocId(input.parentProjectId, "parentProjectId"), deps);
  const records = [...deps.projectGraph].map(([id, data]) => ({ id, data }));
  const projects = records
    .map(({ id, data }) => buildProjectSummary(data, id))
    .filter((project) => canReadTaskRecord(project, deps))
    .filter((project) => !Object.hasOwn(input, "parentProjectId") || project.parentProjectId === normalizeString(input.parentProjectId))
    .filter((project) => !selectedIds || selectedIds.has(project.projectId))
    .filter((project) => !status || project.status === status)
    .filter(project => status || input.includeArchived === true || !getCollaborativeBranch(project, deps) || project.status !== "archived")
    ...
    .sort((a, b) => {
      ...
      return (b.updatedAt || "").localeCompare(a.updatedAt || "") || a.projectId.localeCompare(b.projectId);
    });
  const { page, hasMore, nextCursor } = resultPage(projects, input, "projectId", allResults ? projects.length : limit);
...
async function getProject(input = {}, deps = {}) {
  ...
  const project = buildProjectSummary(doc.data() || {}, projectId);
  if (input.includeDescendants !== true) return { project };
  return { project, hierarchy: await buildProjectHierarchy(projectId, input, deps) };
}

async function updateProject(input = {}, deps = {}) {
  if (input.changes?.permanentlyDelete === true) {
    if (Object.keys(input.changes).some(key => !["permanentlyDelete", "confirmDelete"].includes(key))) throw createProjectTaskError("Deletion cannot be combined with edits", 400, {}, "invalid_delete_request");
    return require("./task-record-deletion").deleteTaskRecord({ recordType: "project", recordId: input.projectId, expectedVersion: input.expectedVersion, confirmDelete: input.changes.confirmDelete }, deps);
  }
  const projectsCollection = getProjectCollection(deps);
  const projectId = validateDocId(input.projectId, "projectId");
  const docRef = projectsCollection.doc(projectId);
  ...
  const nextProject = {
    ...
    teamId: normalizeTeamId(existing.teamId)
  };
  ...
  if (existing.maintenanceManagers?.length && Object.hasOwn(changes, "branchMembers")) {
    if (existing.maintenanceManagers.some(subject => !(changes.branchMembers || []).some(member => member.subject === subject && member.role === "editor"))) throw hierarchyError("Revoke Maintenance manager authority with setMaintenanceAccess before removing its editor membership");
  }
  const hierarchyChanged = Object.hasOwn(changes, "parentProjectId") && normalizeString(changes.parentProjectId) !== (existing.parentProjectId || "");
  if (Object.hasOwn(changes, "parentProjectId")) nextProject.parentProjectId = normalizeParentProjectId(changes.parentProjectId);
  ...
  const teamChanged = Object.prototype.hasOwnProperty.call(changes, "teamId") &&
    normalizeTeamId(changes.teamId) !== (nextProject.teamId || "");
  if (Object.prototype.hasOwnProperty.call(changes, "teamId")) {
    nextProject.teamId = normalizeTeamId(changes.teamId);
  }
  ...
  if (Object.prototype.hasOwnProperty.call(changes, "status")) {
    ...
    if (nextProject.status === "archived" && existing.status !== "archived") {
      nextProject.archivedBySub = actor.actorSub;
      nextProject.archivedByName = actor.actorName;
      nextProject.lastArchivedAt = nowIso;
  ...
  if (Object.hasOwn(changes, "collaborationPolicy")) {
    assertCanManageBranch(existing, deps);
    if (!["editor_archive_creator_delete", "editor_archive_owner_delete"].includes(changes.collaborationPolicy) || existing.visibility !== "branch") throw hierarchyError("Invalid branch collaboration policy");
    nextProject.collaborationPolicy = changes.collaborationPolicy;
  }
  const sharingChanged = Object.hasOwn(changes, "branchMembers") || Object.hasOwn(changes, "visibility") && (existing.visibility === "branch" || nextProject.visibility === "branch");
  if (sharingChanged) {
    assertCanManageBranch(existing, deps);
    if (Object.hasOwn(changes, "branchMembers")) nextProject.branchMembers = normalizeBranchMembers(changes.branchMembers);
  }
  if (nextProject.visibility === "branch" && !["work", "church"].includes(nextProject.lifeArea)) throw hierarchyError("Branch projects belong to work or church");
  ...
  if ((hierarchyChanged || sharingChanged) && (departmentChanged || teamChanged)) throw hierarchyError("Move the project separately from changing its team or department");
  let updatedTaskCount = 0;
  if (hierarchyChanged || sharingChanged) {
    await saveHierarchyProject({ collection: projectsCollection, next: nextProject, snapshot: doc,
      authorize: graph => assertHierarchyChangeAllowed(nextProject, existing, { ...deps, projectGraph: graph }) });
  } else updatedTaskCount = await saveProjectWithDepartmentCascade({
    docRef,
    nextProject,
    projectId,
    departmentChanged: departmentChanged || teamChanged,
    snapshot: doc,
    deps,
    nowIso,
    actor
  ...
  return {
    ...
    departmentCascade: {
      changed: departmentChanged || teamChanged,
      updatedTaskCount,
      teamChanged
    }
  };
...
async function recheckMaintenanceWrite(transaction, record, deps, kind, creating = false) {
  if (!isMaintenanceProject(record.projectId, deps.projectGraph)) return;
  const snapshot = await transaction.get(getProjectCollection(deps).orderBy("__name__").limit(10001));
  if (snapshot.docs.length > 10000) throw hierarchyError("Project context exceeds read budget", "query_requires_narrower_scope", 422);
  const projectGraph = new Map(snapshot.docs.map(doc => [doc.id, { ...doc.data(), projectId: doc.id }]));
  const parent = projectGraph.get(record.projectId);
  if (!parent || !isMaintenanceProject(record.projectId, projectGraph)) throw hierarchyError("Maintenance project moved or was removed");
  assertCanUpdateTaskRecord(creating ? parent : record, {}, { ...deps, projectGraph }, creating ? "project" : kind);
}
...
async function createTask(input = {}, deps = {}) {
  ...
  if (sourceMessageId) {
    ...
    if (matchingSource) {
      throw createProjectTaskError(
        "This source record is already linked to a task",
        409,
  ...
  if (accessibleProject?.visibility === "branch") {
    assertCanUpdateTaskRecord(accessibleProject, {}, deps, "project");
    input = { ...input, visibility: input.visibility === "private" ? "private" : "branch" };
  } else if (input.visibility === "branch") throw hierarchyError("Branch tasks require a branch project");
  ...
  const projectTeamId = projectId ? normalizeTeamId(accessibleProject?.teamId) : "";
  if (projectId && requestedTeamId && requestedTeamId !== projectTeamId) {
    throw createProjectTaskError("A task in a project must use the project's team", 409, {
      projectId, projectTeamId, requestedTeamId
    }, "task_project_team_conflict");
  }
  assertCanCreateTaskRecord({
    ...input,
    teamId: projectId ? projectTeamId : requestedTeamId,
    visibility: input.visibility || (accessibleProject ? getRecordVisibility(accessibleProject) : input.visibility)
  }, deps, "task");
  ...
  const actor = getTaskActorFields(deps);
  // An explicit blank is intentional (for example, an unassigned imported item).
  // Preserve the usual self-assignment only when the caller omits the field.
  const assignedTo = Object.prototype.hasOwnProperty.call(input, "assignedTo")
    ? normalizeString(input.assignedTo) : actor.actorName || "Dan";
  const assignedToSub = Object.prototype.hasOwnProperty.call(input, "assignedToSub")
    ? normalizeString(input.assignedToSub) : actor.actorSub;
  const assignedToEmail = Object.prototype.hasOwnProperty.call(input, "assignedToEmail")
    ? normalizeString(input.assignedToEmail) : actor.actorEmail;
  ...
  const maintenance = input.maintenance === undefined ? null : normalizeMaintenance(input.maintenance);
  validateMaintenanceParent(maintenance, projectId, deps.projectGraph);
  if (maintenance?.workflowStatus === "Completed" && status !== "done") throw hierarchyError("Completed maintenance work requires task status done");
  if (maintenance && status === "done") maintenance.workflowStatus = "Completed";
  const task = {
    maintenance,
    taskId,
    title,
    ...
    lastArchivedAt: status === "dropped" ? nowIso : "",
    archivedBySub: status === "dropped" ? actor.actorSub : "",
    archivedByName: status === "dropped" ? actor.actorName : "",
    statusBeforeDrop: "",
    restoredAt: "",
    restoredBySub: "",
    restoredByName: ""
  ...
  task.searchText = buildTaskSearchText(task);
  if (task.projectId && tasksCollection.firestore?.runTransaction) {
    await tasksCollection.firestore.runTransaction(async tx => {
      const parent = await tx.get(getProjectCollection(deps).doc(task.projectId));
      if (!parent.exists) throw createProjectTaskError("Project no longer exists", 409, {}, "project_not_found");
      await recheckMaintenanceWrite(tx, task, deps, "task", true);
      tx.create(docRef, task);
    });
  } else await docRef.create(task);
...
async function listTasks(input = {}, deps = {}, allResults = false) {
  const tasksCollection = getTaskCollection(deps);
  if (input.includeDescendants && !input.projectId) throw hierarchyError("Choose a project for its descendant tasks");
  let selectedProjectIds;
  if (input.projectId && input.includeDescendants === true) {
    deps = await withProjectContext(deps);
    await assertProjectReferenceAccessible(validateDocId(input.projectId, "projectId"), deps);
    selectedProjectIds = branchIds(input.projectId, deps.projectGraph);
  }
  const limit = normalizeLimit(input.limit);
  ...
  const tasks = records
    ...
    .filter(task => status || input.includeArchived === true || !getCollaborativeBranch(task, deps) || task.status !== "dropped")
    .filter((task) => !priority || task.priority === priority)
    .filter((task) => !projectId || (selectedProjectIds ? selectedProjectIds.has(task.projectId) : task.projectId === projectId))
    ...
    .sort((a, b) => {
      ...
      return (b.updatedAt || "").localeCompare(a.updatedAt || "") || a.taskId.localeCompare(b.taskId);
    });
  const { page, hasMore, nextCursor } = resultPage(tasks, input, "taskId", allResults ? tasks.length : limit);

  return {
    count: page.length,
    detailLevel, hasMore, nextCursor,
    tasks: detailLevel === "compact" ? page.map(buildCompactTask) : page
  };
}
...
async function updateTask(input = {}, deps = {}) {
  if (input.changes?.permanentlyDelete === true) {
    if (Object.keys(input.changes).some(key => !["permanentlyDelete", "confirmDelete"].includes(key))) throw createProjectTaskError("Deletion cannot be combined with edits", 400, {}, "invalid_delete_request");
    return require("./task-record-deletion").deleteTaskRecord({ recordType: "task", recordId: input.taskId, expectedVersion: input.expectedVersion, confirmDelete: input.changes.confirmDelete }, deps);
  }
  const tasksCollection = getTaskCollection(deps);
  const taskId = validateDocId(input.taskId, "taskId");
  const docRef = tasksCollection.doc(taskId);
  ...
  const assignmentOnlyAccess = !hasProjectChange && (isRecordOwner(existing, deps) || isAccessSubject(existing.assignedToSub, getTaskAccess(deps)));
  const referencedProject = assignmentOnlyAccess ? deps.projectGraph?.get(nextProjectId) || null : await assertProjectReferenceAccessible(nextProjectId, deps);
  if (hasProjectChange && referencedProject?.visibility === "branch") assertCanUpdateTaskRecord(referencedProject, {}, deps, "project");
  if (hasProjectChange && isBranchRecord(existing, deps) && !isTaskAdmin(deps) && !isRecordOwner(existing, deps)) throw createProjectTaskError("Only the task owner or administrator may move work out of its shared branch", 403, {}, "project_sharing_denied");
  const nowIso = getNowIso(deps);
  const actor = getTaskActorFields(deps);
  const nextTask = {
    ...clone(existing),
  ...
  if (Object.hasOwn(changes, "maintenance")) nextTask.maintenance = normalizeMaintenance(changes.maintenance);
  validateMaintenanceParent(nextTask.maintenance, nextProjectId, deps.projectGraph);
  ...
  const hasTeamChange = Object.prototype.hasOwnProperty.call(changes, "teamId");
  const requestedTeamId = hasTeamChange ? normalizeTeamId(changes.teamId) : normalizeTeamId(nextTask.teamId);
  if (nextProjectId) {
    const projectTeamId = normalizeTeamId(referencedProject?.teamId);
    if (hasTeamChange && requestedTeamId !== projectTeamId) {
      throw createProjectTaskError("A task in a project must use the project's team", 409, {
        projectId: nextProjectId, projectTeamId, requestedTeamId
      }, "task_project_team_conflict");
    }
    nextTask.teamId = projectTeamId;
  } else if (hasTeamChange) {
    nextTask.teamId = requestedTeamId;
  }
  ...
  if (Object.prototype.hasOwnProperty.call(changes, "status")) {
    ...
    if (nextTask.status === "dropped" && existing.status !== "dropped") {
      nextTask.archivedBySub = actor.actorSub;
      nextTask.archivedByName = actor.actorName;
      nextTask.lastArchivedAt = nowIso;
  ...
  if (referencedProject?.visibility === "branch" && nextTask.visibility !== "private") nextTask.visibility = "branch";
  if (nextTask.visibility === "branch" && referencedProject?.visibility !== "branch") {
    if (hasProjectChange) nextTask.visibility = "private";
    else throw hierarchyError("Branch tasks require a branch project");
  }
  if (nextTask.maintenance) {
    if (nextTask.status === "done") nextTask.maintenance.workflowStatus = "Completed";
    else if (existing.status === "done" && !Object.hasOwn(changes, "maintenance")) nextTask.maintenance.workflowStatus = "Not Started";
    else if (nextTask.maintenance.workflowStatus === "Completed" && nextTask.status !== "dropped") throw hierarchyError("Completed maintenance work requires task status done");
  }
  ...
  nextTask.searchText = buildTaskSearchText(nextTask);
  if (nextTask.projectId && tasksCollection.firestore?.runTransaction) {
    await tasksCollection.firestore.runTransaction(async tx => {
      const parent = await tx.get(getProjectCollection(deps).doc(nextTask.projectId));
      if (!parent.exists) throw createProjectTaskError("Project no longer exists", 409, {}, "project_not_found");
      await recheckMaintenanceWrite(tx, existing, deps, "task");
      tx.update(docRef, nextTask, { lastUpdateTime: doc.updateTime });
    });
  } else await saveObservedRecord(docRef, nextTask, doc);
...
async function createRoutine(input = {}, deps = {}) {
  ...
  const projectId = normalizeString(input.projectId);
  const project = projectId ? await assertProjectReferenceAccessible(projectId, deps) : null;
  if (projectId) {
    if (!isMaintenanceProject(projectId, deps.projectGraph) || project?.visibility !== "branch") throw hierarchyError("Project routines currently require the Maintenance branch");
    assertCanUpdateTaskRecord(project, {}, deps, "project");
    input = { ...input, visibility: "branch", lifeArea: "church" };
  }
  ...
  if (routine.projectId && routinesCollection.firestore?.runTransaction) {
    await routinesCollection.firestore.runTransaction(async tx => {
      await recheckMaintenanceWrite(tx, routine, deps, "routine", true);
      tx.create(docRef, routine);
    });
  } else await docRef.create(routine);
...
async function listRoutines(input = {}, deps = {}, allResults = false) {
  const routinesCollection = getRoutineCollection(deps);
  const limit = normalizeLimit(input.limit);
  const status = normalizeString(input.status);
  const lifeArea = normalizeString(input.lifeArea);
  ...
  const routines = records
    ...
    .slice(0, allResults ? undefined : limit);
...
async function updateRoutine(input = {}, deps = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(changes, "status")) {
    ...
    if (nextRoutine.status === "archived" && existing.status !== "archived") {
      nextRoutine.archivedBySub = actor.actorSub;
      nextRoutine.archivedByName = actor.actorName;
      nextRoutine.lastArchivedAt = nowIso;
  ...
  if (Object.hasOwn(changes, "projectId") && changes.projectId !== existing.projectId) throw hierarchyError("Routine project moves require a reviewed migration");
  if (Object.hasOwn(changes, "maintenance")) nextRoutine.maintenance = normalizeMaintenance(changes.maintenance);
  validateMaintenanceParent(nextRoutine.maintenance, nextRoutine.projectId, deps.projectGraph);
  if (nextRoutine.visibility === "branch" && !isMaintenanceProject(nextRoutine.projectId, deps.projectGraph)) throw hierarchyError("Branch routines require Maintenance");
  if (existing.projectId && nextRoutine.visibility !== "branch") throw hierarchyError("Maintenance routines retain branch visibility");
  ...
  if (nextRoutine.projectId && routinesCollection.firestore?.runTransaction) {
    await routinesCollection.firestore.runTransaction(async tx => {
      await recheckMaintenanceWrite(tx, existing, deps, "routine");
      tx.update(docRef, nextRoutine, { lastUpdateTime: doc.updateTime });
    });
  } else await saveObservedRecord(docRef, nextRoutine, doc);
...
async function restoreTaskRecord(input = {}, deps = {}) {
  ...
  const access = getTaskAccess(deps);
  if (!["system", "admin", "manager", "member"].includes(access.role)) {
    throw createProjectTaskError(
      "A task editor is required to restore archived work",
      403,
  ...
  if (access.role === "member" && !(getCollaborativeBranch(record, deps) && getBranchRole(record, deps) === "editor")) throw createProjectTaskError("Members may restore work only in an editor-archive branch", 403, {}, "task_restore_denied");
  if (isBranchRecord(record, deps)) assertCanUpdateTaskRecord(record, {}, deps, recordType);
  if (
    access.role === "manager"
    && !["staff", "branch"].includes(getRecordVisibility(record))
...
async function listStaffProfiles(input = {}, deps = {}) {
  ...
  const profiles = (await loadCollection(getStaffProfileCollection(deps), 500))
    ...
    .filter((profile) => !query || [profile.displayName, profile.email, profile.role, ...profile.leadershipAreas].join(" ").toLowerCase().includes(query))
...
async function updateStaffProfile(input = {}, deps = {}) {
  ...
  const allowedFields = selfService
    ...
    : ["displayName", "email", "role", "status", "managerSub", "leadershipAreas", "weeklyCapacityMinutes", "sharePrivateCapacity"];
  ...
  if (Object.prototype.hasOwnProperty.call(changes, "leadershipAreas")) {
    next.leadershipAreas = normalizeLeadershipAreas(changes.leadershipAreas);
  }
...
async function respondToAssignment(input = {}, deps = {}) {
  ...
  await saveObservedRecord(docRef, next, doc);
  let notification = null;
...
async function buildDailyReview(input = {}, deps = {}) {
  ...
  const [projectsResult, tasksResult, routinesResult, notificationsResult] = await Promise.all([
    listProjects({ status: "active" }, deps, true),
    listTasks({}, deps, true),
    listRoutines({ status: "active" }, deps, true),
  ...
  return {
    ...
    summary: {
      ...
      activeNextCount: nextTasks.length,
    ...
    activeNext: nextTasks.map(mapTask).map((task, index) => ({
      ...task,
...
function buildLeadershipIdentityResolution(staffProfiles = [], authorizationProfiles = []) {
  const activeAuthorizationProfiles = authorizationProfiles
    .filter((profile) => profile.status === "active")
    .map((profile) => ({
      ...profile,
      identitySubjects: [...new Set([
        normalizeString(profile.subject),
        ...(Array.isArray(profile.identitySubjects) ? profile.identitySubjects.map(normalizeString) : [])
      ].filter(Boolean))].sort()
    }));
  const duplicateGroups = new Map();
  for (const profile of activeAuthorizationProfiles) {
    const email = normalizeString(profile.email).toLowerCase();
    const duplicateKey = email
      ? `${email}\u0000${profile.identitySubjects.join("\u0000")}`
      : `profile:${profile.profileId || profile.subject}`;
    const group = duplicateGroups.get(duplicateKey) || [];
    group.push(profile);
    duplicateGroups.set(duplicateKey, group);
  }

  const identityGroups = [...duplicateGroups.values()].map((profiles) => {
    const ordered = [...profiles].sort((left, right) => {
      const leftIsEntra = normalizeString(left.subject).startsWith("entra|") ? 1 : 0;
      const rightIsEntra = normalizeString(right.subject).startsWith("entra|") ? 1 : 0;
      return rightIsEntra - leftIsEntra
        || normalizeString(right.lastSeenAt).localeCompare(normalizeString(left.lastSeenAt))
        || normalizeString(left.subject).localeCompare(normalizeString(right.subject));
    });
    return {
      canonicalProfile: ordered[0],
      aliases: [...new Set(ordered.flatMap((profile) => profile.identitySubjects))].sort()
    };
  });
  ...
  const conflicts = [...aliasOwners.entries()]
    .filter(([, owners]) => owners.length > 1)
    .map(([alias]) => alias);
  if (conflicts.length > 0) {
    throw createProjectTaskError(
      "Leadership reporting stopped because an identity alias is claimed by more than one staff authorization profile",
      409,
      { conflictingSubjects: conflicts },
      "staff_identity_alias_conflict"
    );
  }

  const canonicalBySubject = new Map();
  const authorizationByCanonicalSubject = new Map();
  for (const group of identityGroups) {
    const canonicalSubject = group.canonicalProfile.subject;
    authorizationByCanonicalSubject.set(canonicalSubject, group.canonicalProfile);
    for (const alias of group.aliases) canonicalBySubject.set(alias, canonicalSubject);
  }
  const resolveSubject = (subject) => {
    const cleanSubject = normalizeString(subject);
    return canonicalBySubject.get(cleanSubject) || cleanSubject;
  };

  const profilesByCanonicalSubject = new Map();
  for (const profile of staffProfiles.filter((item) => item.status === "active")) {
    const canonicalSubject = resolveSubject(profile.subject);
    const candidates = profilesByCanonicalSubject.get(canonicalSubject) || [];
    candidates.push(profile);
    profilesByCanonicalSubject.set(canonicalSubject, candidates);
  }
  const canonicalStaffProfiles = [...profilesByCanonicalSubject.entries()].map(([canonicalSubject, profiles]) => {
    const ordered = [...profiles].sort((left, right) => {
      const leftIsCanonical = left.subject === canonicalSubject ? 1 : 0;
      const rightIsCanonical = right.subject === canonicalSubject ? 1 : 0;
      return rightIsCanonical - leftIsCanonical
        || normalizeString(right.lastSeenAt).localeCompare(normalizeString(left.lastSeenAt))
        || normalizeString(left.subject).localeCompare(normalizeString(right.subject));
    });
    const selected = ordered[0];
    const authorizationProfile = authorizationByCanonicalSubject.get(canonicalSubject);
    return {
      ...selected,
      subject: canonicalSubject,
      displayName: authorizationProfile?.displayName || selected.displayName,
      email: authorizationProfile?.email || selected.email
    };
  });
  return { canonicalStaffProfiles, resolveSubject };
}
...
async function buildLeadershipBrief(input = {}, deps = {}) {
  ...
  const [taskRecords, projectRecords, staffProfileRecords, staffAuthorizationProfileRecords] = await Promise.all([
    ...
    loadCollection(deps.staffAuthorizationProfilesCollection, 500)
  ]);
  const allTasks = taskRecords
    .map(({ id, data }) => buildTaskSummary(data, id))
    .filter((task) => !["done", "dropped"].includes(task.status));
  const tasks = taskRecords
    .filter(({ data }) => canReadTaskRecord(data, deps) && ["staff", "branch"].includes(getRecordVisibility(data)))
    .map(({ id, data }) => buildTaskSummary(data, id))
    .filter((task) => !["done", "dropped"].includes(task.status));
  ...
  const rawStaffProfiles = staffProfileRecords
    .map(({ id, data }) => buildStaffProfileSummary(data, id));
  const authorizationProfiles = staffAuthorizationProfileRecords
    .map(({ id, data }) => buildStaffAuthorizationSummary(data, id));
  const { canonicalStaffProfiles: staffProfiles, resolveSubject } = buildLeadershipIdentityResolution(
    rawStaffProfiles,
  ...
  for (const task of tasks) {
    const personKey = resolveSubject(task.assignedToSub) || task.assignedToEmail || task.assignedTo || "unassigned";
    const person = people.get(personKey) || {
      personKey,
      name: task.assignedTo || task.assignedToEmail || "Unassigned",
      subject: resolveSubject(task.assignedToSub),
  ...
  for (const task of allTasks.filter((item) => item.visibility === "private")) {
    const personKey = resolveSubject(task.assignedToSub || task.ownerSub);
...
async function buildDailyBrief(input = {}, deps = {}) {
  ...
  const lines = [
    ...
    `Summary: ${review.summary.openTaskCount} open tasks, ${review.summary.activeNextCount} active next actions, ${review.summary.needsReviewCount} needing overdue review, ${review.summary.dueTodayCount} due today, ${review.summary.highPriorityNextCount} high-priority next actions, ${review.summary.highPriorityProjectCount} high-priority projects, ${review.summary.projectTargetDueCount} project targets due, ${review.summary.followUpDueCount} waiting follow-ups due, ${review.summary.sarahRequestedOpenCount} Sarah-requested open items, ${review.summary.eventCompletedTaskCount} event-bound tasks auto-completed.`,
    "",
    "Needs review:",
    ...
    ...review.activeNext.filter((item) => item.lifeArea === "work").map((item) => `${item.displayNumber}. Active${item.priority === "high" ? " (high priority)" : ""}: ${item.title}`),
    ...
    ...review.activeNext.filter((item) => item.lifeArea === "home").map((item) => `${item.displayNumber}. Active${item.priority === "high" ? " (high priority)" : ""}: ${item.title}`),
    ...
    ...review.activeNext.filter((item) => item.lifeArea === "church").map((item) => `${item.displayNumber}. Active${item.priority === "high" ? " (high priority)" : ""}: ${item.title}`),
    ...
    ...review.activeNext.filter((item) => item.lifeArea === "personal").map((item) => `${item.displayNumber}. Active${item.priority === "high" ? " (high priority)" : ""}: ${item.title}`),
  ...
  return {
    ...
    activeNextCount: review.summary.activeNextCount,
...
// All public task/project entry points share one fresh project graph per call.
// Notes, reviews and legacy REST callers use the same authorization context.
for (const name of ["createRoutine", "listRoutines", "updateRoutine", "createProject", "listProjects", "getProject", "updateProject", "createTask", "getTask", "listTasks", "updateTask", "addTaskNote", "listTaskNotes", "buildDailyReview", "buildDailyBrief", "buildLeadershipBrief", "respondToAssignment", "restoreTaskRecord"]) {
  const handler = module.exports[name];
  module.exports[name] = async (input, deps = {}, ...rest) => handler(input, await withProjectContext(deps), ...rest);
}
