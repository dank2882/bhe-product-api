...
async function buildDailyReview(input = {}, deps = {}) {
  ...
  const eventCompletion = await completeTasksForPastEvents({}, deps);
  ...
  const eventsResult = await listCalendarEvents(
    { fromDate: today, toDate: today, status: "scheduled", limit: 100 },
    deps
  );
  ...
  const mapEvent = detailLevel === "full" ? (event) => event : buildBriefEvent;
  ...
  return {
    ...
    summary: {
      ...
      eventCompletedTaskCount: eventCompletion.completedCount,
      projectsWithoutNextActionCount: projectsWithoutNextAction.length
    },
    eventCompletedTasks: eventCompletion.completedTasks.map(mapTask),
    eventsToday: eventsResult.events.map(mapEvent),
