...
function isPersonalRecordForAccess(record = {}, access = {}) {
  ...
  if (!subject && !email && !name) return true;
  if (subject && [record.assignedToSub, record.ownerSub, record.createdBySub, record.leadSub]
    .map(normalizeString).includes(subject)) return true;
...
async function restoreTaskRecord(input = {}, deps = {}) {
  ...
  if (access.role === "manager" && getRecordVisibility(record) !== "staff" && record.ownerSub !== access.subject) {
    throw createProjectTaskError(
...
async function updateStaffProfile(input = {}, deps = {}) {
  const access = getTaskAccess(deps);
  const subject = normalizeString(input.subject);
  const selfService = input.selfService === true && subject === access.subject;
...
async function listMyNotifications(input = {}, deps = {}) {
  ...
  const notifications = (await loadCollection(getNotificationCollection(deps), 1000))
    ...
    .filter((item) => item.recipientSub === access.subject)
...
async function markNotificationRead(input = {}, deps = {}) {
  ...
  const existing = doc.data() || {};
  if (existing.recipientSub !== access.subject && !["system", "admin"].includes(access.role)) {
...
async function respondToAssignment(input = {}, deps = {}) {
  ...
  const existing = doc.data() || {};
  const access = getTaskAccess(deps);
  if (!access.subject || normalizeString(existing.assignedToSub) !== access.subject) {
  ...
  await docRef.set(next);
  let notification = null;
  if (existing.assignedBySub && existing.assignedBySub !== access.subject) {
    notification = await createTaskNotification({
...
async function buildDailyReview(input = {}, deps = {}) {
  ...
  const pendingAssignments = access.subject
    ? openTasks.filter((task) => task.assignedToSub === access.subject && task.assignmentStatus === "proposed")
    : [];
  const assignmentQuestions = allOpenTasks.filter((task) =>
    task.assignmentStatus === "needs_clarification" &&
    (!access.subject || task.assignedToSub === access.subject || task.assignedBySub === access.subject)
  );
