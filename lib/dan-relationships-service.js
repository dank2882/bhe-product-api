...
async function updatePerson(input = {}, deps = {}) {
  ...
  return runDanFirestoreTransaction(deps, collection, async (transaction) => {
    ...
    if (!document.exists) throw createRelationshipError("person not found", 404, "person_not_found", { id: input.personId });
    const existing = { ...(document.data() || {}), personId: input.personId };
...
async function updateContactMethod(input = {}, deps = {}) {
  ...
  const docRef = collection.doc(input.contactMethodId);
  return runDanFirestoreTransaction(deps, collection, async (transaction) => {
    const document = await transaction.get(docRef);
    if (!document.exists) throw createRelationshipError("contactMethod not found", 404, "contactMethod_not_found", { id: input.contactMethodId });
    const existing = { ...(document.data() || {}), contactMethodId: input.contactMethodId };
