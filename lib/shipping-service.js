...
function mutation(s, operation, input, actor, timestamp, prepared) {
  ...
  else if (operation === "uploadDocument") { if (s.documents.some(d => d.id === prepared.id)) M.fail("Document ID already exists; use a new version ID"); s.documents.push(prepared); materialEdit(s); }
...
async function runShippingCommand(operation, input, key, deps) {
  ...
  const prepared = !prior.exists && operation === "uploadDocument" ? await prepareFile(input, deps) : null;
...
module.exports = { access, listShipments, getShipment, getReadiness, getCostSummary, getDocumentChecklist, getSchedule, getHistoryInsights, getShipmentHistory, getDocument, previewImportBatch, commitImportBatch, runShippingCommand, commandSections };
