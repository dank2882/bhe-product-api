...
function isThoughtOwner(thought = {}, deps = {}) {
  const actorSubjects = [...getActorSubjects(deps), ...delegatedOwnerSubjects(getTaskAccess(deps).subject, deps.privateDelegationEnv)];
...
function buildThoughtRecord(thought = {}, fallbackId = "") {
  return {
    ...
    updatedBySub: thought.updatedBySub || "",
    updatedByName: thought.updatedByName || "",
...
async function updateThinkTankEntry(input = {}, deps = {}) {
  ...
  const editor = getTaskActorFields(deps);
  next.updatedBySub = editor.actorSub;
  next.updatedByName = editor.actorName;
