...
const PRAYER_OPERATIONS = Object.freeze([
  ...
  define("getTodaysPrayers", "query", "Get active prayers due today in their configured time zones, ordered in Dan's stable Upward, Inward, and Outward format with numbered subheaders and notes.", [], ["at"], service.getTodaysPrayers),
  ...
  define("createPrayer", "command", "Create an encrypted private prayer. Defaults to daily; weekly, monthly, interval, and one-date schedules are optional. Legacy unscheduled input is treated as daily.", ["listId", "title", "prayerText"], ["privateContext", "tags", "people", "topics", "schedule", "presentation", "source", "sourceRef", "prayerId"], service.createPrayer),
  define("updatePrayer", "command", "Update prayer content, list, or daily, weekly, monthly, interval, or one-date schedule with optimistic concurrency.", ["prayerId", "expectedVersion", "changes"], [], service.updatePrayer),
...
const CATALOG_VERSION = "2026-09-25";
...
async function runPrayerManagementOperation(input = {}, deps = {}) {
  ...
  if (mode === "command" && deps.prayerAuditEventsCollection) {
    ...
    await deps.prayerAuditEventsCollection.doc(auditId).create({ auditId, ownerSub: auditOwner.subject, actorSub: auditOwner.actorSubject, actorName: auditOwner.name, operation: name, argumentKeys: Object.keys(args).sort(), resultKeys: Object.keys(result || {}).sort(), createdAt: timestamp instanceof Date ? timestamp.toISOString() : String(timestamp), contentRedacted: true });
  }
